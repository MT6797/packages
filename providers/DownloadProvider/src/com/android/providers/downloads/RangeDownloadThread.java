...
public class RangeDownloadThread implements Runnable {
    ...
    @Override
    public void run() {
        Log.v(Constants.TAG, "Range download thread run, downloadId: " + mInfo.mId
                + ", blockId: " + mBlockId);
        ...
        try {
            ...
            Log.e(Constants.TAG, "Range download thread abort, downloadId: " + mInfo.mId
        ...
        Log.v(Constants.TAG, "Range download thread done, downloadId: "
    ...
    private void executeRangeDownload() throws StopRequestException {
        if (mCurrentWriteBytes == mTotalLength) {
            Log.v(Constants.TAG, "Range download aleady done, will return, downloadId: "
                    + mInfo.mId + ", blockId: " + mBlockId);
            return;
        }
    ...
    private void transferData(HttpResponse response) throws StopRequestException {
        ...
        try {
            try {
                ...
                Log.v(Constants.TAG, "transfer data, seek happen exception: " + e.toString());
            ...
            try {
                ...
                Log.v(Constants.TAG, "download file close happen exception: " + e.toString());
            }
        }
    ...
    private void writeDatatoDestination(RandomAccessFile downloadFile, byte[] data, int off,
        ...
        try {
            ...
            Log.v(Constants.TAG, "writeDatatoDestination happen io exception, mInfo.mId: "
                    + mInfo.mId + ",blockId: " + mBlockId);
            throw new StopRequestException(STATUS_FILE_ERROR, e);
        }
    }
    ...
    private int calcuateBufferSize() {
        ...
        Log.d(Constants.TAG, "Total Len:" + mTotalLength + ":"
        ...
        Log.d(Constants.TAG, "Buffer Size:" + bufferSize + ":" + defaultBufferSize);
        return bufferSize;
    }
}
