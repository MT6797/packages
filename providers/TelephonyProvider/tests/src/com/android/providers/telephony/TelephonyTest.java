...
public class TelephonyTest extends AndroidTestCase {
    ...
    private static final Uri SMS_UNDELIVERED = Uri
            .parse("content://sms/undelivered");
    private static final Uri SMS_FAILED = Uri.parse("content://sms/failed");
    private static final Uri SMS_CONVERSATIONS = Uri
            .parse("content://sms/conversations");
    private static final Uri SMS_STATUS_PENDING = Uri
            .parse("content://sms/sr_pending");
    private static final Uri SMS_ALL_THREADID = Uri
            .parse("content://sms/all_threadid");
    private static final Uri SMS_UNREAD = Uri
            .parse("content://mms-sms/unread_count");
    ...
    private static final Uri URI_CONVERSATIONS_MESSAGES = Uri
            .parse("content://mms-sms/conversations");
    private static final Uri URI_OBSOLETE_THREADS = Uri
            .parse("content://mms-sms/conversations/obsolete");
    private static final Uri URI_CANONICAL_ADDRESS = Uri
    ...
    private static final String DB_PATH
            = "/data/data/com.android.providers.telephony/databases/mms_test.zip";
    ...
    protected void setUp() throws Exception {
        ...
        AppOpsManager appOps = (AppOpsManager) getContext().getSystemService(
                Context.APP_OPS_SERVICE);
        appOps.setMode(AppOpsManager.OP_WRITE_SMS, 1001,
                "com.android.providers.telephony", AppOpsManager.MODE_ALLOWED);
    }
    ...
    public void testCase01_insertSms() {
        ...
        Log.d(TAG, "testCase01 end resultUri = " + insertSmsUri + " id= "
                + insertSmsId);
    }
    ...
    public void testCase11_unreadSms() {
        ...
        Log.d(TAG, "testCase11 unread After= " + unreadAfter + " unread="
                + unread);
        deleteMsg(insertSmsUri);
    ...
    public void testCase13_insertMms() {
        /*
         * Log.d(TAG, "testCase_insertMms"); /// M: fix zip file PIM issue //
         * byte[] pduMms = readFileContent(DB_PATH, DB_NAME); byte[] pduMms =
         * readFile("mmspdu"); assert (pduMms != null); PduPersister persister =
         * PduPersister.getPduPersister(mContext); RetrieveConf retrieveConf =
         * null; try { retrieveConf = (RetrieveConf) new
         * PduParser(pduMms).parse(); insertMmsUri =
         * persister.persist(retrieveConf, Mms.Inbox.CONTENT_URI, true, false,
         * null); /** M: insertMmsUri is Mms.Inbox.CONTENT_URI append msgid
         * assert (insertMmsUri != null); Log.d(TAG,
         * "testCase_insertMms insertMmsUri = " + insertMmsUri); } catch
         * (Exception e) { Log.d(TAG, "testCase_insertMms Exception e");
         * e.printStackTrace(); }
         */
    }

    /**
     * M: for query a mms, after persist a mms pdu. depend on testcase13.
     */
    @InternalApiAnnotation
    ...
    /**
     * M: for update a mms, after persist a mms pdu. depend on testcase13.
     */
    @InternalApiAnnotation
    ...
    /**
     * M: for delete a mms, after persist a mms pdu. depend on testcase13.
     */
    @InternalApiAnnotation
    ...
    public void testCase19_getOrCreateThreadId() {
        ...
        long threadid = Threads.getOrCreateThreadId(getContext(), ADDRESS);
        assert (threadid > 0);
        deleteMsg(ContentUris.withAppendedId(URI_CONVERSATIONS_MESSAGES,
                threadid));
    ...
    protected Cursor queryMsg(Uri uri, long smsId) {
        return mResolver.query(ContentUris.withAppendedId(uri, smsId), null,
                null, null, null);
    }
    ...
    protected void updateSms(Uri uri) {
        ...
        try {
            ...
            String body = cursor.getString(cursor
                    .getColumnIndexOrThrow(Sms.BODY));
    ...
    protected void assertSms(Cursor cursor) {
        try {
            ...
            int threadId = cursor.getInt(cursor
                    .getColumnIndexOrThrow("thread_id"));
            String address = cursor.getString(cursor
                    .getColumnIndexOrThrow(Sms.ADDRESS));
            String body = cursor.getString(cursor
                    .getColumnIndexOrThrow(Sms.BODY));
            Long dates = cursor.getLong(cursor.getColumnIndexOrThrow(Sms.DATE));
            long subId = cursor.getLong(cursor
                    .getColumnIndexOrThrow(Telephony.Sms.SUBSCRIPTION_ID));
            int read = cursor.getInt(cursor.getColumnIndexOrThrow(Sms.READ));
            String serviceCenter = cursor.getString(cursor
                    .getColumnIndexOrThrow(Sms.SERVICE_CENTER));
    ...
    protected int deleteSms(String id) {
        Uri deleteUri = ContentUris.withAppendedId(Sms.CONTENT_URI, Long
                .parseLong(id));
