package com.mediatek.contacts.sne;
        ...
        if (!isSneNicknameValid(sNickname, subId)) {
            {
                if (checkAlone) {
                    ...
                    uiHandler.post(new Runnable() {
                        public void run() {
                            LogUtils.d(TAG,"[validateAndUpdateSne]" +
                                "return false as nickname is Invalid toast");
                            Toast.makeText(mHostAppContext,
                                    mContext.getString(R.string.nickname_too_long),
