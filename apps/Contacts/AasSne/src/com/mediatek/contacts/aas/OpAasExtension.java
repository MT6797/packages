...
import com.mediatek.contacts.LogUtils;
import com.mediatek.contacts.R;
...
public class OpAasExtension extends DefaultAasExtension {
    ...
    public void updatePhoneType(int subId, DataKind kind) {
        ...
        kind.fieldList.add(new EditField(Phone.NUMBER, mhostAppContext.getResources()
    ...
    private void ensureKindExists(RawContactDelta state, String mimeType,
        ...
        if (kind != null) {
            ...
            if (values != null && values.size() == slotAnrSize + 1) {
                ...
                LogUtils.d(TAG, "ensureAASKindExists() size=" + values.size() + " slotAnrSize="
            ...
            if (values == null || values.isEmpty()) {
                LogUtils.d(TAG, "ensureKindExists() Empty, insert primary:1 and anr:" +
                    slotAnrSize);
                ...
                LogUtils.d(TAG, "ensureKindExists() pnrSize=" + pnrSize + ", anrSize="
                        + slotAnrSize);
                if (pnrSize < 1) {
                    ...
                    if (type == STRING_PRIMART) {
                        ((TextView) view)
                                .setHint(mContext
                                        .getResources()
                                        .getString(
                                                R.string.aas_phone_primary));
                    } else if (type == STRING_ADDITINAL) {
                        ((TextView) view)
                                .setHint(mContext
                                        .getResources()
                                        .getString(
                                                R.string.aas_phone_additional));
                    }
                } else {
                    LogUtils.e(TAG,
        ...
        case VIEW_UPDATE_LABEL:
            if (isBack && SimUtils.isUsim(accountType)) {
                ...
                DataKind dataKind = accType
                        .getKindForMimetype(Phone.CONTENT_ITEM_TYPE);
                // get subId here & pass to below API,
                // as host has already called the setcurrentsubid before entry editor
    ...
    public boolean onTypeSelectionChange(RawContactDelta rawContact,
        ...
        if (SimUtils.isUsim(accountType) && SimUtils.isPhone(kind.mimeType)) {
            ...
            if (type == select) {
                LogUtils.i(TAG, "same select");
                return true;
            }
            if (Phone.TYPE_CUSTOM == select.rawValue) {
                LogUtils.i(TAG, "[onTypeSelectionChange] Custom Selected");
                ...
                // para
                LogUtils.i(TAG, "[onTypeSelectionChange] different Selected");
    ...
    private void onTypeSelectionChange(int position) {
        ...
        if (SimUtils.isUsim(SimUtils.getCurAccount())) {
            ...
            int subId = SimUtils.getCurSubId();
            LogUtils.d(TAG, " onTypeSelectionChange internal: subId to fill in slot_key= " + subId);
    ...
    public boolean updateValues(Intent intent, int subId,
        ...
        newSimData = intent.getParcelableArrayListExtra(KEY_SIM_DATA);
        ArrayList<RawContactDelta> oldSimData = new ArrayList<RawContactDelta>();
        oldSimData = intent.getParcelableArrayListExtra(KEY_OLD_SIM_DATA);
