...
class PanoramaThumbGetter {
    ...
    private static final String BACK_GROUND_COLOR = "#1A1A1A";
    private static final int MILLI_SECOND_PER_WAIT = 200;
    ...
    public PanoramaThumbGetter(MediaData data, ThumbType thumbType,
        ...
        if (bitmap == null) {
            MtkLog.i(TAG, "<new> rotate/resize bitmap fail");
            mInitFail = true;
            return;
        }
        // prepare config
    ...
    public Bitmap getThumbnail(int index) {
        ...
        synchronized (task) {
            while (!task.isDone()) {
                try {
                    task.wait(MILLI_SECOND_PER_WAIT);
