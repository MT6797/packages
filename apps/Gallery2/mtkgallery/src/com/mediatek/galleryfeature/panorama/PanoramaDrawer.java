...
public class PanoramaDrawer {
    ...
    private static final float PERSPECTIVE_Z_NEAR = 0.1f;
    private static final float PERSPECTIVE_Z_FAR = 1000.0f;
    ...
    public synchronized void drawOnTexture(MGLCanvas canvas, MRawTexture targetTexture,
            float degree) {
        if (mFree) {
    ...
    public synchronized Bitmap drawOnBitmap(MGLCanvas canvas, MRawTexture targetTexture,
            float degree) {
        if (mFree) {
    ...
    private synchronized Bitmap readPixels(MGLCanvas canvas) {
        ...
        for (int i = 0; i < mPanoramaConfig.mCanvasHeight; i++) {
            for (int j = 0; j < mPanoramaConfig.mCanvasWidth; j++) {
                ...
                bt[(mPanoramaConfig.mCanvasHeight - i - 1) * mPanoramaConfig.mCanvasWidth + j]
                        = pix1;
            }
        }
    ...
    private void draw(MGLCanvas canvas, float degree, boolean isBitmapTarget) {
        ...
        canvas.setPerspective(mPanoramaConfig.mFovy, (float) mPanoramaConfig.mCanvasWidth
                / (float) mPanoramaConfig.mCanvasHeight, PERSPECTIVE_Z_NEAR, PERSPECTIVE_Z_FAR);
        if (isBitmapTarget) {
