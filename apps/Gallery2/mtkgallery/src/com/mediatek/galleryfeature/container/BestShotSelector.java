...
public class BestShotSelector {
    ...
    private static final int BEST_SHOT_COUNT_ADJUST_PARAMS = 10;
    ...
    public BestShotSelector(Context context, ArrayList<MediaData> mediaDataArray) {
        ...
        mBestShotNum = mEntryArray.size() / BEST_SHOT_COUNT_ADJUST_PARAMS;
    ...
    public void markBestShot() {
        ...
        for (Entry entry : mEntryArray) {
            int mark = entry.mWave == Entry.WAVE_CREST ?
                    MediaData.BEST_SHOT_MARK_TRUE : MediaData.BEST_SHOT_MARK_FALSE;
        ...
        for (Entry entry : mEntryArray) {
            ...
            if (entry.mWave == Entry.WAVE_CREST) {
                ...
                if (state == WAIT_FIRST_CREST) {
                    ...
                    MtkLog.i(TAG, "<mergeWaveCrest> index = " + firstCrest + ",start = "
                            + fvlist1Start + ", end = " + fvlist1End + ", average = " + average1);
                    MtkLog.i(TAG, "<mergeWaveCrest> index = " + secondCrest + ",start = "
                            + fvlist2Start + ", end = " + fvlist2End + ", average = " + average2);
                    ...
                    if (average.compareTo(average1) < 0 || average.compareTo(average2) < 0) {
                        if (mEntryArray.get(firstCrest).mFocusValue.compareTo(mEntryArray
                            ...
                            MtkLog.i(TAG, "<mergeWaveCrest> set index = " + firstCrest
                                    + " as WAVE_NORMAL");
                            mEntryArray.get(firstCrest).mWave = Entry.WAVE_NORMAL;
                        } else {
                            MtkLog.i(TAG, "<mergeWaveCrest> set index = " + secondCrest
                                    + " as WAVE_NORMAL");
