...
public class DrmPlayer extends Player implements Player.OnFrameAvailableListener {
    ...
    // using to record rights status when onPrepare
    private boolean mHasRightsToShow;
    ...
    public boolean onPrepare() {
        ...
        mHasRightsToShow = DrmHelper.hasRightsToShow(mContext, mMediaData);
        ...
        prepareDisplayLimit();
        MtkLog.d(TAG, " <onPrepare> prepareSuccess = "
                + prepareSuccess
                + " rights = "
                + DrmHelper.checkRightsStatus(mContext, mMediaData.filePath,
    ...
    public void consumeRights() {
        if (mConsumeStatus == RIGHTS_CONSUMED)
            return;
        boolean hasRightsToShowNew = DrmHelper.hasRightsToShow(mContext, mMediaData);
        if (mHasRightsToShow != hasRightsToShowNew) {
            MtkLog.i(TAG, "<consumeRights> old mHasRightsToShow = " + mHasRightsToShow + ", new = "
                    + hasRightsToShowNew + ", rePrepareWhenRightsChanged");
            rePrepareWhenRightsChanged();
            mHasRightsToShow = hasRightsToShowNew;
        }
    ...
    private void rePrepareWhenRightsChanged() {
        if (mTextureAfterConsume != null) {
            mTextureAfterConsume.recycle();
            mTextureAfterConsume = null;
        }
