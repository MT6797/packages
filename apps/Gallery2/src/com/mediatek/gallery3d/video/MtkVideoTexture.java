...
public class MtkVideoTexture extends VideoSurfaceTexture implements ScreenModeListener,
    ...
    public void prepare() {
        ...
        if (!surface.isValid()) {
            MtkLog.v(TAG, "surface is inValid, cancel prepare");
            return;
        }
        ...
        try {
            ...
            MtkLog.e(TAG, "IllegalStateException, Unable to open content: " + mUri, ex);
            mErrorListener.onError(mMediaPlayer, MediaPlayer.MEDIA_ERROR_UNKNOWN, 0);
            return;
        }
    }
    ...
    private void startVideo() {
        ...
        // if is end, do not start video
        if(isEnd()) {
            ((Activity) mContext).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    MovieControllerOverlay.getMovieController(mContext).showPaused();
                }
            });
            mPosition = 0;
    ...
    /**
     * Because pre-roll has been removed on M0 version, video will seek to last I frame,
     * and automatically play from the last I frame if resumed. To avoid the issue,
     * don't start video if position is close to total duration
     * @return boolean
     *      if(duration - position < 500ms), return true
     */
    private boolean isEnd() {
        MtkLog.d(TAG, "isEnd, duration: " + getDuration() + ", mPosition: " + mPosition);
        return getDuration() - mPosition < 500;
    }
