...
import com.mediatek.gallery3d.video.MhlPowerSaving;
...
public class MoviePlayer implements
    ...
    ///@}
    ///M: Add for MHL @{
    private final Runnable mMhlStartHideRunnable;
    private static final String MHL_EXTENSION_MODE_SUPPORT = "1";
    ///@}
    ...
    // /M: mFirstHideSystemUiRunnable is for status bar and navigation
    // bar not show after mute and play a very short video {
    private Runnable mFirstHideSystemUiRunnable;
    // /@}
    ...
    ///M: for mhl power saving.
    private static MhlPowerSaving mMhlPowerSaving;
    ///M: for hotKnot
    ...
    private int mAudiofocusState = AudioManager.AUDIOFOCUS_REQUEST_GRANTED;
    ///M: add telephony manager to manage telephone state
    private TelephonyManager mTelePhoneManager;
    private static final int REQUEST_AUDIOFOCUS_DELAY = 1000;
    ...
    // /M: When call coming before video start play, audio focus will not be granted,
    // for the case, request audio focus again after call dismiss @{
...
//    };
    ...
    public MoviePlayer(View rootView, final MovieActivity movieActivity, IMovieItem info,
        ...
        // /@}
        ///M: Turn screen off if MHL is connected.  @{
        mMhlStartHideRunnable = new Runnable() {
            @Override
            public void run() {
                if (mMhlPowerSaving.isMhlConnected()) {
                    mMhlPowerSaving.startCountDown();
                }
            }
        };
        ///@}
...
//        });
        ...
        mZoomController = new VideoZoomController(movieActivity, mVideoRoot,
            ...
            public void onDownEvent() {
                ...
                ///M: If downEvent happened with MHL connected,
                 //   light on screen and turn it off after about 3500mm @{
                if (mMhlPowerSaving != null && mMhlPowerSaving.isMhlConnected()){
                    mMhlPowerSaving.cancelCountDown();
                    mHandler.removeCallbacks(mMhlStartHideRunnable);
                    mHandler.postDelayed(mMhlStartHideRunnable,HIDE_SYSTEM_UI_DELAY);
                }
                /// @}
            }
            ...
            public boolean isInWfdExtension() {
                return (mWfdPowerSaving != null && mWfdPowerSaving.isInExtensionMode())
                       || (mMhlPowerSaving !=null && mMhlPowerSaving.isMhlConnected());
            }
        };
        //}@
        ...
        */
        ...
        // MoviePlayer is both used for normal video and slide video. But the looper is necessary
        // when new PhoneStateListener, the slideVideo is created in sub thread, it has no looper.
        // so new PhoneStateListener at the special constructor only for normal video
        mPhoneStateListener = new PhoneStateListener() {
            @Override
            public void onCallStateChanged(int state, String inComingNumber){
                Log.v(TAG,"onCallStateChanged state= " + state + "mAudiofocusState= " + mAudiofocusState);
                if (state == TelephonyManager.CALL_STATE_IDLE) {
                    //if has not gaint audiofocus, request it
                    if (mAudiofocusState == AudioManager.AUDIOFOCUS_REQUEST_FAILED) {
                        mHandler.postDelayed(mRequestAudioFocusRunnable,REQUEST_AUDIOFOCUS_DELAY);
                    }
                }
            }
        };
    ...
    private void showSystemUi(boolean visible, boolean isFirstOpen) {
        ...
        if (!visible) {
            ...
            if (isFirstOpen && isLocalFile) {
                mFirstHideSystemUiRunnable = new Runnable() {
                    @Override
                    public void run() {
                        Log.v(TAG, "mFirstHideSystemUiRunnable, showSystemUi() flag = " + flagx);
                ...
                mRootView.postDelayed(mFirstHideSystemUiRunnable, SHOW_ACTIONBAR_TIME);
    ...
    public boolean onPause() {
        ...
        ///M: for unregister phone state listener
        if(mTelePhoneManager != null && mPhoneStateListener != null) {
            Log.v(TAG, "cancel listen PhoneStateListener");
            mTelePhoneManager.listen(mPhoneStateListener, PhoneStateListener.LISTEN_NONE);
        }
        ///M: for diss mhl presentation @{
        if (mMhlPowerSaving != null) {
            Log.v(TAG, " MhlConnected= " + mMhlPowerSaving.isMhlConnected());
            mMhlPowerSaving.unregisterMhlReceiver();
            if (mMhlPowerSaving.isMhlConnected()) {
                mMhlPowerSaving.cancelCountDown();
                mMhlPowerSaving.dismissPresentaion();
            }
        }
        /// @}
        if (mWfdPowerSaving != null) {
    ...
    public void onResume() {
        ...
        mHandler.post(mProgressChecker);
        startSlowMotionChecker();
        //add listen to telephone state
        if(mPhoneStateListener != null) {
            Log.v(TAG, "listen PhoneStateListener");
    ...
    private final Runnable mRequestAudioFocusRunnable = new Runnable(){
        @Override
        public void run() {
            requestAudioFocus();
        }
    ...
    private void requestAudioFocus() {
        mAudiofocusState = ((AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE)).requestAudioFocus(
                this, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN);
        Log.v(TAG, "requestAudioFocus mAudiofocusState= " + mAudiofocusState);
    }
  ///@}

  /// M: enable Mhl Extension Mode when Mhl is connected @{
    private void enableMhlPowerSavingIfNeed() {
        boolean isEnableExtensionMode = MHL_EXTENSION_MODE_SUPPORT.equals(SystemProperties.get("mtk_hdmi_ext_mode"));
        if (LOG) {
            Log.v(TAG, "enableMhlPowerSavingIfNeed()" + " isEnableExtensionMode = " + isEnableExtensionMode);
        }
        initMhlPowerSaving();
        if (isEnableExtensionMode) {
            mMhlPowerSaving.registerMhlReceiver();
        }
    }

    private void initMhlPowerSaving() {
        if (LOG) {
            Log.v(TAG, "initMhlPowerSaving()" + " mMhlPowerSaving= " + mMhlPowerSaving);
        }
        if (mMhlPowerSaving == null || !mActivityContext.equals(mMhlPowerSaving.getCurrentActivity())) {
            mMhlPowerSaving =
                    new MhlPowerSaving(mRootView, mActivityContext, mVideoView, mHandler) {
                @Override
            ...
            mMhlPowerSaving.refreshMhlPara();
        }
    }
  ///@}
  ...
  /// @}
    ...
    public void onShown() {
        ...
        // /}@
        mRootView.setBackgroundColor(Color.BLACK);
        mRootView.removeCallbacks(mFirstHideSystemUiRunnable);
        mShowing = true;
        setProgress();
...
//        });
    ...
    public void onPrepared(MediaPlayer mp) {
      ...
      /// M: for continue from last break point, continue play video when MHL inserted and
      // pause video when MHL removed @{
        if (mMhlPowerSaving != null && mMhlPowerSaving.isSurfaceRemoved()) {
            Log.v(TAG, " mhlSurfaceRemoved= " + mMhlPowerSaving.isSurfaceRemoved()
                    + " mhlCurrentPosition= " + mMhlPowerSaving.getMhlCurrentPosition()
                    + " isPlayingBeforeExtMode= " + mMhlPowerSaving.isPlayingBeforeExtMode());
            mVideoView.seekTo(mMhlPowerSaving.getMhlCurrentPosition());
            if(mMhlPowerSaving.isMhlConnected() && mMhlPowerSaving.isPlayingBeforeExtMode()) {
                mVideoView.start();
                mController.showPlaying();
            } else {
                mVideoView.pause();
            ...
            mMhlPowerSaving.setIsSurfaceRemoved(false);
        }
      ///@}
    ...
    public boolean onInfo(MediaPlayer mp, int what, int extra) {
        ...
        if (what == MediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START) {
            ...
            // /M: query fps must be after media player started, to avoid abnormal
            // slowmotion icon, update again after video rendering start
            if (mSlowMotionItem.isSlowMotionVideo() && mController != null) {
                Log.v(TAG, "MEDIA_INFO_VIDEO_RENDERING_START, updateSlowMotionIcon");
                mController.updateSlowMotionIcon();
            }
        }
        // / @}
