    ...
    private void updateSlowMotionInfoToDBIfNeed(final Context context, final Uri sourceVideoUri, final Uri newVideoUri) {
        ...
        if (sourceItem.isSlowMotionVideo()) {
            ...
            if (duration < sourceItem.getSectionEndTime()) {
                ...
                endTime = (int)((long)duration * sourceItem.getSectionEndTime() / sourceItem.getDuration());
            } else {
                endTime = sourceItem.getSectionEndTime();
            }
            Log.v(TAG, "[updateSlowMotionInfoToDBIfNeed] muted file duration = " + duration
                + ", sourceItem.getSectionEndTime() = " + sourceItem.getSectionEndTime()
                + ", sourceItem.getDuration() = " + sourceItem.getDuration()
                + ", endTime = " + endTime);
