...
/// @}
    ...
    /// @}
        ...
        /// M: [FEATURE.ADD] VTSP @{
        boolean hasDraw = drawCurrentSlotDynamic(entry.coverItem, canvas, index, width, height,
                entry.rotation);
        if (!hasDraw) {
        /// @}
    ...
    private boolean drawCurrentSlotDynamic(MediaItem item, GLCanvas canvas, int index, int width,
            int height, int rotation) {
        if (!mSupportVTSP || item == null) {
            return false;
        }
        // draw dynamic
        canvas.save(GLCanvas.SAVE_FLAG_MATRIX);
