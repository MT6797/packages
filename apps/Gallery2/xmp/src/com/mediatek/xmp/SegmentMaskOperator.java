...
public class SegmentMaskOperator {
    private static final String TAG = "MtkGallery2/Xmp/SegmentMaskOperator";
    ...
    public static class SegmentMaskInfo {
        ...
        public String toString() {
            ...
            String str2 = null;
            if (mMaskBuffer != null) {
                sb.append("\n    mMaskBuffer length = 0x" + Integer.toHexString(mMaskBuffer.length)
                        + "(" + mMaskBuffer.length + ")");
            } else {
                sb.append("\n    mMaskBuffer = null");
            }
     ...
     * @param maskInfo
     *            mask info
     * @return suceess->true,fail->false
     */
    ...
    public SegmentMaskInfo getSegmentMaskInfoFromFile(String filePath) {
        ...
        if (mEnableDump && segmentMaskInfo.mMaskBuffer != null) {
            XmpInterface.writeBufferToFile(mDumpPath + "SegmentMaskBuffer_Read.raw",
                    segmentMaskInfo.mMaskBuffer);
        }
        if (mEnableDump) {
            XmpInterface.writeStringToFile(mDumpPath + "SegmentMaskInfo_Read.txt", segmentMaskInfo
                    .toString());
        }
    ...
    private byte[] getSegmentMaskBufferFromSections(RandomAccessFile rafIn) {
        try {
            ...
            for (; i < mParsedSectionsForGallery.size(); i++) {
                ...
                if (sec.mIsSegmentMask) {
                    rafIn.seek(sec.mOffset + 2 + 2);
                    dataLen = rafIn.readInt();
                    Log.d(TAG, "<getSegmentMaskBufferFromSections> type SEGMASK, dataLen: 0x"
            ...
            for (i = i - 1; i < mParsedSectionsForGallery.size(); i++) {
                ...
                if (sec.mIsSegmentMask) {
                    ...
                    copyLen = app1Len - 2 - XmpInterface.TOTAL_LENGTH_TAG_BYTE
                            - XmpInterface.TYPE_SEGMENT_MASK.length()
                            - XmpInterface.SEGMENT_SERIAL_NUM_TAG_BYTE;
    ...
    private boolean writeSegmentMaskInfoToJpg(String srcFilePath, String dstFilePath,
        ...
        if (mEnableDump && maskInfo.mMaskBuffer != null) {
            XmpInterface.writeBufferToFile(mDumpPath + "SegmentMaskBuffer_Written.raw",
                    maskInfo.mMaskBuffer);
        }
        if (mEnableDump) {
            XmpInterface.writeStringToFile(mDumpPath + "SegmentMaskInfo_Written.txt", maskInfo
                    .toString());
        }
        ...
        try {
            ...
            writeSegmentMask(rafIn, rafOut, srcFilePath, maskInfo);
    ...
    private void writeSegmentMask(RandomAccessFile rafIn, RandomAccessFile rafOut,
            String srcFilePath,
            SegmentMaskInfo maskInfo) throws IOException {
        ...
        for (int i = 0; i < mParsedSectionsForGallery.size(); i++) {
            ...
            if (sec.mIsExif) {
                ...
                if (!hasWritenSegmentMaskData && (sec.mMarker == XmpInterface.DQT
                        || sec.mMarker == mXmpInterface.DHT)) {
                ...
                } else if (needUpdateMaskBuffer && sec.mIsSegmentMask) {
                    // delete depth data
                    rafIn.skipBytes(sec.mLength + 2);
                } else {
                    mXmpInterface.writeSectionToFile(rafIn, rafOut, sec);
    ...
    private boolean updateOnlySegmentMaskInfoWoBuffer(RandomAccessFile rafOut, XMPMeta meta,
        ...
        if (newXmpMainBuffer == null) {
            Log.d(TAG, "<updateOnlySegmentMaskInfoWoBuffer> updated xmp main data is null," +
                    "return false!!!");
            return false;
        }
        try {
    ...
    private byte[] updateXmpMainDataWithSegmentMaskInfo(XMPMeta meta, SegmentMaskInfo maskInfo) {
        if (maskInfo == null || meta == null) {
            Log.d(TAG, "<updateXmpMainDataWithSegmentMaskInfo> maskInfo or meta is null," +
                    "so return null!!!");
            return null;
        }
        mXmpInterface.registerNamespace(XmpInterface.MEDIATEK_SEGMENT_NAMESPACE,
        ...
        if (bufferOutTmp == null) {
            Log.d(TAG, "<updateXmpMainDataWithSegmentMaskInfo> bufferOutTmp is null,"
                    + "so return null!!!");
            return null;
        }
    ...
    private boolean writeOnlySegmentMaskBuffer(RandomAccessFile rafOut, SegmentMaskInfo maskInfo) {
        ...
        if (maskInfo.mMaskBuffer == null) {
            Log.d(TAG,
                    "<writeOnlySegmentMaskBuffer> mMaskBuffer is null, skip write mMaskBuffer!!!");
            return true;
        }
        try {
            int totalCount = 0;
            ...
            if (segmentMaskArray == null) {
                Log.d(TAG, "<writeOnlySegmentMaskBuffer> segmentMaskArray is null,"
                        + "skip write mMaskBuffer!!!");
                return true;
            }
            for (int i = 0; i < segmentMaskArray.size(); i++) {
                byte[] section = segmentMaskArray.get(i);
                if (section[INDEX_0] == 'S' && section[INDEX_1] == 'E' && section[INDEX_2] == 'G'
                        && section[INDEX_3] == 'M') {
                    // current section is depth buffer
                    totalCount = maskInfo.mMaskBuffer.length;
                    Log.d(TAG, "<writeOnlySegmentMaskBuffer> write mMaskBuffer total count: 0x"
                            + Integer.toHexString(totalCount));
                }
                rafOut.writeShort(XmpInterface.APP15);
                rafOut.writeShort(section.length + 2 + APP15_LENGTHTAG_BYTE_COUNT);
    ...
    private ArrayList<byte[]> makeSegmentMaskData(byte[] maskData) {
        ...
        byte[] data = maskData;
        String header = XmpInterface.TYPE_SEGMENT_MASK;
        int dataRemain = data.length;
        int dataOffset = 0;
        int sectionCount = 0;

        while (header.length() + 1 + dataRemain >= XmpInterface.SEGMENT_MASK_PACKET_SIZE) {
            byte[] section = new byte[XmpInterface.SEGMENT_MASK_PACKET_SIZE];
            // copy type
        ...
        if (header.length() + 1 + dataRemain < XmpInterface.SEGMENT_MASK_PACKET_SIZE) {
