...
public class CallerInfo {
    ...
    */
    ...
    private static String getCurrentCountryIso(Context context, Locale locale, int subId) {
        ...
        /// M: [WFC] The countryIso might be empty while making a wifi call when airplane mode on @{
        final boolean isAirplaneModeOn =
                Settings.System.getInt(context.getContentResolver(),
                Settings.System.AIRPLANE_MODE_ON, 0) != 0;
        if (TextUtils.isEmpty(countryIso) && isAirplaneModeOn) {
            CountryDetector detector = (CountryDetector) context.getSystemService(
                   Context.COUNTRY_DETECTOR);
            Log.d(TAG, " CountryDetector detector " + detector);
            if (detector != null) {
        ...
        /// @}
        } else if (subId == Call.INVALID_SUB_ID || TextUtils.isEmpty(countryIso)) {
