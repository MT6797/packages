...
public class LosslessReceiver extends BroadcastReceiver {
    ...
    private static final String CLOSE_LOSSLESS_NOTIFICATION =
            SoundEnhancement.CLOSE_LOSSLESS_NOTIFICATION;
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        if (LOSSLESS_CLOSE.equals(action)) {
            Log.d("@M_" + TAG, "close the lossless.");
            cancelNotification(mNotificationManager, LOSSLESS_ICON_ID);
        } else if (LOSSLESS_PLAYING.equals(action)) {
            Log.d("@M_" + TAG, "playing the lossless.");
            createNotification(mNotificationManager, R.drawable.bt_audio_play,
                    audioProfileIntent, context, true, R.string.lossless_playing);
        } else if (LOSSLESS_ADD.equals(action)) {
            Log.d("@M_" + TAG, "open the lossless.");
            createNotification(mNotificationManager, R.drawable.bt_audio,
                    audioProfileIntent, context, false, R.string.lossless_on);
        } else if (LOSSLESS_STOP.equals(action)) {
            Log.d("@M_" + TAG, "stop the lossless.");
            createNotification(mNotificationManager, R.drawable.bt_audio,
                    audioProfileIntent, context, false, R.string.lossless_volume_max);
        } else if (CLOSE_LOSSLESS_NOTIFICATION.equals(action)) {
            Log.d("@M_" + TAG, "close the notification lossless.");
            ((AudioManager) context.getSystemService(Context.AUDIO_SERVICE))
                    .setParameters(SET_LOSSLESSBT_DISABLED);
        } else if (LOSSLESS_NOT_SUPPORT.equals(action)) {
            Log.d("@M_" + TAG, "cannot found the lossless device.");
            createNotification(mNotificationManager, R.drawable.bt_audio,
                    audioProfileIntent, context, false, R.string.lossless_cannot_found_device);
        }
    }
    ...
    private void createNotification(NotificationManager mNotificationManager, int icon,
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(),
                R.layout.lossless_notification);
