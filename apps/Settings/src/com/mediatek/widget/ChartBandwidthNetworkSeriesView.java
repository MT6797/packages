...
public class ChartBandwidthNetworkSeriesView extends View {
    ...
    public void generatePath() {
        ...
        for (int i = 0 ; i < mStats.size(); i++) {
            ...
            Log.d("@M_" + TAG, "index = " + i + ", rxBytes = "
                + entry.rxBytes + ", txBytes = " + entry.txBytes);
            totalData += entry.rxBytes + entry.txBytes;
        }

        Log.d("@M_" + TAG, "totalData = " + totalData + ", mTotalUsed = " + mTotalUsed);

        currentData = mTotalUsed == 0 ? 0 : totalData - mTotalUsed;
        mTotalUsed = totalData;
        Log.d("@M_" + TAG, "currentData = " + currentData);
