...
public class ContactInfoHelper {
    ...
    public static ContactInfo getContactInfo(Cursor c) {
        ...
        /// M: [Union Query] Add union query feature support. @{
        if (null == c) {
        ...
        if (DialerFeatureOptions.CALL_LOG_UNION_QUERY) {
            ...
            info.name = c.getString(CallLogQuery.CALLS_JOIN_DATA_VIEW_DISPLAY_NAME);
            info.label = c
                    .getString(CallLogQuery.CALLS_JOIN_DATA_VIEW_CALL_NUMBER_TYPE);
            info.photoId = c.getLong(CallLogQuery.CALLS_JOIN_DATA_VIEW_PHOTO_ID);
            String photo = c.getString(CallLogQuery.CALLS_JOIN_DATA_VIEW_PHOTO_URI);
            info.photoUri = (null == photo) ? null : Uri.parse(photo);
            info.normalizedNumber = info.number; // TODO format number
            info.formattedNumber = info.number; // TODO format number
            info.contactSimId = c
                    .getInt(CallLogQuery.CALLS_JOIN_DATA_VIEW_INDICATE_PHONE_SIM);
            info.contactId = c.getInt(CallLogQuery.CALLS_JOIN_DATA_VIEW_CONTACT_ID);
            String lookUp = c.getString(CallLogQuery.CALLS_JOIN_DATA_VIEW_LOOKUP_KEY);
            info.lookupUri = (info.contactId == 0) ? null : Contacts
                    .getLookupUri(info.contactId, lookUp);
            // In case lookupUri be NULL, set a temporary uri according call number
            if (info.lookupUri == null && !TextUtils.isEmpty(info.number)) {
                info.lookupUri = ContactInfoHelper.createTemporaryContactUri(info.number);
            }
            info.rawContactId = c
                    .getInt(CallLogQuery.CALLS_JOIN_DATA_VIEW_RAW_CONTACT_ID);
            info.isSdnContact = c
                    .getInt(CallLogQuery.CALLS_JOIN_DATA_VIEW_IS_SDN_CONTACT);
        } else {
