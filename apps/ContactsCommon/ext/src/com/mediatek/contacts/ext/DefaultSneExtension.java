...
public class DefaultSneExtension implements ISneExtension {
    ...
    /**
     * called to update the data in contact DB for SNE
     *
     * @param intent
     *            The intent to start SIMEditProcessor,it contains the data to
     *            be edit
     * @param indexInSim
     *            the index in sim of contact
     * @param subId
     *            sub id current edit on
     * @param rawContactId
     */
    @Override
    public void editSimSne(Intent intent, long indexInSim, int subId, long rawContactId) {
        ...
        // if is insert,sames should set the value of
        // newTag/newNumber/newEmail/newAnr same as tag/number/email/anr ?
        // if is update,sames should set the value of tag/number/email/anr same
        // as newTag/numNumber/newEmail/newAnr ?
    }

    /**
     * called to check nickname is empty or not or invalid
     *
     * @param which
     *            processor call checkNickName()
     * @param intent
     *            The intent start SIMEditProcessor
     * @param canShowToast
     *            true can,false not show toast by plugin
     * @param subId
     *            sub id
     * @return 0 if NickName is ok,1 for empty,2 for too long
     */
    @Override
    public int checkNickName(ProcessorBase processor, Intent intent, boolean canShowToast,
            int subId) {
    ...
    public void copySimSneToAccount(ArrayList<ContentProviderOperation> operationList,
        ...
        // 2.buildInsertOperation for
        // simNickname,like:buildOperation(account.type, operationList, null,
        // nickname, backRef);
    }
