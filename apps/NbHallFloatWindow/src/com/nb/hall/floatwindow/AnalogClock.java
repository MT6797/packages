  ...
  //      Log.d(TAG, "2--------:x: "+x+"   y: "+y);
   ...
   //     Log.d(TAG, "3--------:hight: "+getHeight());
       ...
       //     canvas.drawCircle(x, y - (h / 2) + mDotOffset, mDotRadius, mDotPaint);
        ...
        canvas.drawText(Integer.toString(mDay), 3*x/2-mDayBorader.getIntrinsicWidth()/4, y+mDayBorader.getIntrinsicHeight()/4, mPaint);
    ...
    private void drawPoint(Canvas canvas, Drawable hand, int x, int y,
            boolean changed) {
        canvas.save();
        if (changed) {
    ...
    private void drawSubBg(Canvas canvas, Drawable hand, int x, int y,
            boolean changed, float scale) {
        canvas.save();
