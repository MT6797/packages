...
public class Mailbox extends EmailContent implements EmailContent.MailboxColumns, Parcelable {
    ...
    public static Mailbox newSystemMailbox(Context context, long accountId, int mailboxType) {
        ...
        switch (mailboxType) {
            ...
            case TYPE_DRAFTS:
                flags = Mailbox.FLAG_HOLDS_MAIL;
                /// M: Support of DRAFT Sync if requested. @{
                IOperatorAccount mExt = (IOperatorAccount) MPlugin.createInstance(
                        IOperatorAccount.class.getName(), context);
                if (mExt == null) {
                    mExt = new DefaultOperatorAccount();
                }
                if (mExt.isDraftSyncNeeded(context, accountId)) {
                    syncInterval = 0;
                } else {
                    syncInterval = Account.CHECK_INTERVAL_NEVER;
                }
