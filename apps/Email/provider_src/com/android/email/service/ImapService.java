...
public class ImapService extends Service {
    ...
    private synchronized static void synchronizeMailboxGeneric(final Context context,
        ...
        // 0. We do not ever sync OUTBOX (down or up) but syncing of DRAFT is
        // based on feature requested or not
        ...
        boolean isDraftSync = mExt.isDraftSyncNeeded(context,
                mailbox.mAccountKey);
        boolean flags = isDraftSync ? (mailbox.mType == Mailbox.TYPE_OUTBOX)
                : (mailbox.mType == Mailbox.TYPE_DRAFTS || mailbox.mType == Mailbox.TYPE_OUTBOX);
...
//            final long queryEndDate = endDate - DateUtils.DAY_IN_MILLIS - DateUtils.HOUR_IN_MILLIS;
    ...
    private static void processPendingDataChange(final Context context, Store remoteStore,
        ...
        // 1. No remote update for OUTBOX but for DRAFTS is on request
        IOperatorAccount mExt = OPExtensionFactory
                .getOperatorExtension(context);
        /// M: Support for DRAFT sync if requested
        boolean isDraftSync = mExt.isDraftSyncNeeded(context,
