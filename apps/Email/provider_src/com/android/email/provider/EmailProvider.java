...
//                    }
    ...
    private static boolean uploadsToServer(Context context, Mailbox m) {
        ...
        boolean isDraftSync = mExt.isDraftSyncNeeded(context, m.mAccountKey);
        boolean flags = isDraftSync ? (m.mType == Mailbox.TYPE_OUTBOX
                || m.mType == Mailbox.TYPE_SEARCH) : (m.mType == Mailbox.TYPE_DRAFTS
...
//                                        getContext(), msgId);
    ...
    private int uiDeleteMessage(Uri uri) {
        ...
        IOperatorAccount mExt = OPExtensionFactory
                .getOperatorExtension(context);
        boolean isDraftSync = mExt.isDraftSyncNeeded(context,
                mailbox.mAccountKey);
        boolean flags = isDraftSync ? (mailbox.mType == Mailbox.TYPE_TRASH)
                : (mailbox.mType == Mailbox.TYPE_TRASH || mailbox.mType == Mailbox.TYPE_DRAFTS);
        if (flags) {
