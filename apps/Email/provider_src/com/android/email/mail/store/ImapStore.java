...
public class ImapStore extends Store {
    ...
    public boolean canSyncFolderType(final int type) {
        switch (type) {
            ...
            case Mailbox.TYPE_DRAFTS:
                /// M: Support for DRAFT Sync if requested
                IOperatorAccount mExt = OPExtensionFactory
                        .getOperatorExtension(mContext);
                if (mExt.isDraftSyncNeeded(mContext, mAccount.mId)) {
                    return true;
                }
                return false;
    ...
    public Folder[] updateFolders() throws MessagingException {
        ...
        try {
            ...
            /// M: folders inside INBOX like INBOX/FOLDER_NAME should be mapped
            // to FOLDER LIST at client directly
            IOperatorAccount mExt = OPExtensionFactory
                    .getOperatorExtension(mContext);
            boolean isMapped = mExt.isFolderOverridingRequired(mContext,
                    mAccount.mId);
            for (ImapResponse response : responses) {
                ...
                if (response.isDataResponse(0, xlistSupported ? ImapConstants.XLIST
                    ...
                    if (xlistSupported) {
                        ...
                        if (isMapped) {
                            /// M: add and update mailbox according to the
                            // folder structure of the operator account
                            mExt.updateOperatorMailbox(mContext, mAccount.mId,
                                    mailboxes, folderName, delimiterChar, this);
                        } else {
