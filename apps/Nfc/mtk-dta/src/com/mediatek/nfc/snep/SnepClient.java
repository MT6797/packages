...
public final class SnepClient {
    private static final String TAG = "DTA:SnepClient";
    ...
    private static final String DEFAULT_SNEP_SERVER = "urn:nfc:sn:snep";
    private static final String EXTEND_SNEP_SERVER  = "urn:nfc:sn:sneptest";
    private static final int DEFAULT_SNEP_SAP = 0x04;
    private static final int EXTEND_SNEP_SAP  = 0x13;
    ...
    public void connect() throws IOException {
        ...
        SnepMessenger messenger;
        try {
            if (DBG) Log.d(TAG, "about to create socket , sn:"+mServiceName);
            // Connect to the snep server on the remote side
            int srcSap=0;
            if(mServiceName.equalsIgnoreCase(DEFAULT_SNEP_SERVER)){
                srcSap = DEFAULT_SNEP_SAP;
            }else if(mServiceName.equalsIgnoreCase(EXTEND_SNEP_SERVER)){
                srcSap = EXTEND_SNEP_SAP;
            }

            socket = NfcService.getInstance().createLlcpSocket(srcSap, mMiu, mRwSize, 1024);
            if (socket == null) {
            ...
            Log.d(TAG, "mFragmentLength:"+mFragmentLength+"  RemoteMiu:"+miu);
