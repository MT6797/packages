...
public final class SnepServer {
    private static final String TAG = "DTA:SnepServer";
    ...
    ///-- DTA add  {
    public SnepServer(String serviceName, int serviceSap, Callback callback, int miu) {
        mCallback = callback;
        mServiceName = serviceName;
        mServiceSap = serviceSap;
        mFragmentLength = -1;
    ...
    static boolean handleRequest(SnepMessenger messenger, Callback callback) throws IOException {
        ...
        if (((request.getVersion() & 0xF0) >> 4) != SnepMessage.VERSION_MAJOR) {
            ...
            // DTA
            Log.d(TAG, "unsupported major version, close connection");
            return false;
        } else if (request.getField() == SnepMessage.REQUEST_GET) {
            // DTA
            int acceptableLength = request.getAcceptableLength();
            Log.d(TAG, "acceptableLength: " + acceptableLength);
            if (acceptableLength == (int)0x01f5) {
                Log.d(TAG, "excess data, close connection");
                messenger.sendMessage(SnepMessage.getMessage(SnepMessage.RESPONSE_EXCESS_DATA));
                return false;
            }
        ...
        } else if (request.getField() == SnepMessage.REQUEST_BAD) {

            Log.d(TAG, "TC_S_BIT_BI_02 , REQUEST_BAD request (" + request.getField() +")");
            messenger.sendMessage(SnepMessage.getMessage(
                 SnepMessage.RESPONSE_BAD_REQUEST));

            // DTA
            Log.d(TAG, "return RESPONSE_BAD_REQUEST, close connection");
            return false;
            ...
            // DTA
            Log.d(TAG, "unknown request, close connection");
            return false;
        }
        return true;
    }
