...
public class SnepMessenger {
    private static final String TAG = "DTA:SnepMessager";
    private static final boolean DBG = true;
    private static final int HEADER_LENGTH = 6;
    final LlcpSocket mSocket;
    final int mFragmentLength;
    final boolean mIsClient;
    boolean mSendComplete = false;

    public static final int MAL = 0x19000; //Max Accept length , IXIT

    public SnepMessenger(boolean isClient, LlcpSocket socket, int fragmentLength) {
        Log.d(TAG, "SnepMessenger()  fragmentLength:"+fragmentLength);
        mSocket = socket;
    ...
    public void sendMessage(SnepMessage msg) throws IOException {
        ...
        if (length == buffer.length) {
            mSendComplete = true;
            Log.d(TAG, " sendMessage() send complete ");
            return;
        }
    ...
    public SnepMessage getMessage() throws IOException, SnepException {
        ...
        if (size < 0) {
            ...
            //} catch (IOException e) {
                // Ignore
            //}
            Log.d(TAG, "size < HEADER_LENGTH , not to send fieldReject  ");
        ...
        // DTA
        //TC_C_PUT_BI_01
        if(mSendComplete == true && readSize == 0 &&
            requestSize == 0 && requestField == SnepMessage.RESPONSE_CONTINUE){
            mSendComplete = false;
            throw new IOException("send complete but receive RESPONSE_CONTINUE(0x80)");
        }
        ...
        if (requestSize == -1) {
            ...
            throw new IOException("Inacceptable length -1");
        }else if(requestSize > MAL){
            try {
                mSocket.send(SnepMessage.getMessage(fieldReject).toByteArray());
            } catch (IOException e) {
                // Ignore
            }
            throw new IOException("requestSize length:"+requestSize+" > "+MAL);
