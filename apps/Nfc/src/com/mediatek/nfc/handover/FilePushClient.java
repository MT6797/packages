    ...
    private class ClientSession implements Runnable {
        ...
        private boolean sendFile(Uri uri, SocketChannel channel,
            ...
            if (sendFile == null) {
                ...
                String fileDisplayName = Util.getFileDisplayNameByQuery_dispaly_name(TAG,
                                                                            mContext, uri);
                if (fileDisplayName == null) {
                    Log.e(TAG, "fileDisplayName == null , abort send");
                    return true;
                }

                String wholeFilePath = Util.openFileByInputStream(TAG,
                                            mContext, uri, fileDisplayName);

                sendFile = new File(wholeFilePath);
                if (!sendFile.exists() || !sendFile.isFile()) {
                    Log.e(TAG, " !sendFile.exists() or  !sendFile.isFile(), abort send ");
                return true;
            }
            }
