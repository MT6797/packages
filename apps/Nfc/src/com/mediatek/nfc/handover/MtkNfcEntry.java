...
public class MtkNfcEntry {
     ...
     *      Each individual mode needs to implement this interface
     */
    public interface IP2pLinkManager {
    ...
    public static final String ACTION_DTA_SET_MODE = "com.mediatek.nfc.dta.ACTION_SET_MODE";
    public static final String DTA_EXTRA_P2P_MODE = "com.mediatek.nfc.dta.EXTRA_MODE";
    //public static final int MANAGER_MODE_DEFAULT = 0;
    public static final int P2P_MODE_DTA = 1;


    static private final String DTA_PREF_P2P_MANAGER_MODE = "com.mediatek.nfc.p2pmanagermode";
    /// }
    ...
    public static final int MANAGER_MODE_DTA = 2;
    ...
    boolean mSmallAntennaSupport = false;
    ...
    private MtkNfcEntry(Context context) {
        ...
        mSmallAntennaSupport = NfcRuntimeOptions.isSmallAntennaSupport();
    }
    ...
    public static void init(Context context /*, HandoverManager handoverManager*/) {
        ...
        //}
        if (mMode == MANAGER_MODE_DTA) {

            Log.i(TAG, "MtkNfcEntry return  ");
            return;
        }
    ...
    public NdefMessage createRequestMessage(Uri[] uris) {
        Log.i(TAG, "createRequestMessage() MobileManagerUtils.isSupported(): "
        ...
        try {
            ...
            if (getMode() == MANAGER_MODE_WFA_CERTIFICATION) {
                ...
                boolean inChina = MobileManagerUtils.isSupported();


                if (mCurrentUserId == 0 && mSmallAntennaSupport == false && inChina) {

                    boolean isFileExist = fileExistByUri(mContext, uris[0]);
                    Log.i(TAG, "isFileExist ByUri:" + isFileExist);
                    if (isFileExist == false) {
                        Log.i(TAG, "##  use Android BT Beam  ##");
                        return null;
                    }

                    return MtkHandoverManager.getInstance().CreateHrMEntry(null);
                } else {
                    Log.i(TAG, "mCurrentUserId:" + mCurrentUserId + "  mSmallAntennaSupport:"
                        + mSmallAntennaSupport + " MobileManagerUtils.isSupported():"
                        + inChina + " !!!!  use Android BT Beam !!!!");
                    return null;
                }
            }
        } catch (Exception e) {
    ...
    public int parseHandoverMessage(NdefMessage handoverMessage) {
        ...
        if (mCurrentUserId != 0 || mSmallAntennaSupport == true || inChina == false) {
            Log.i(TAG, "mCurrentUserId:" + mCurrentUserId + " mSmallAntennaSupport:"
                + mSmallAntennaSupport + " inChina:" + inChina + " !!!!  use Android BT Beam !!!!");
    ...
    /*
    *   is file can handle by follwoing method, return true.
    *
    *   return false : use Android Beam(BT)
    *             true : use BeamPlus
    *
    */

    boolean fileExistByUri(Context context, Uri uri) {


        Log.d(TAG, " fileExistByUri()  ");
        try {
            Log.d(TAG, "   uri.toString():" + uri.toString());

            if (uri.getScheme().toString().compareTo("content") != 0) {
                Log.d(TAG, "uri not content return true;");
                return true;
            }


            if (Util.openFileByQuery_data(TAG, context, uri) != null) {
                Log.d(TAG, "openFile by query _data column ,return true;");
                return true;
            }

            Log.d(TAG, "try to openFile by FileInputStream");
            if (Util.fileExistByInputStream(TAG, context, uri)) {
                Log.d(TAG, "openFile by InputStream ,return true;");
                return true;
            }
            ...
            Log.d(TAG, "exception treat as securityException ...");
            e.printStackTrace();
        }
    ...
    static private class ModeChangeListener {
        ...
        public ModeChangeListener(Context context) {
            ...
            filter.addAction(ACTION_DTA_SET_MODE);
            context.registerReceiver(mReceiver, filter);
        }

        public int getMode() {
            Log.d(TAG, "ModeChangeListener.getMode()");

            synchronized (ModeChangeListener.this) {
                int mode = 0;

                if (P2P_MODE_DTA == mPrefs.getInt
                    (DTA_PREF_P2P_MANAGER_MODE, MANAGER_MODE_DEFAULT)) {
                    Log.d(TAG, " DTA_PREF_P2P_MANAGER_MODE == 1 , MANAGER_MODE_DTA");
                    mode = MANAGER_MODE_DTA;
                    return mode;

                }


                mode = mPrefs.getInt(PREF_MTK_HANDOVER_MANAGER_MODE, MANAGER_MODE_INVALID);
        ...
        private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                synchronized (ModeChangeListener.this) {
                    if (action.equals(ACTION_DTA_SET_MODE)) {
                        int mode = intent.getIntExtra(DTA_EXTRA_P2P_MODE, MANAGER_MODE_DEFAULT);
                        mPrefs.edit().putInt(DTA_PREF_P2P_MANAGER_MODE, mode).apply();
                        Log.d(TAG, ACTION_DTA_SET_MODE + ", mode = " + mode);
                    }
