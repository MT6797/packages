...
class P2pLinkManager implements Handler.Callback, P2pEventListener.Callback,
        ...
        synchronized (P2pLinkManager.this)    {
            ...
            if (mMessageToSend != null ||
                ...
                try {
                    if (mUrisToSend != null) {
                        int i = 0;
                        for (Uri printUri: mUrisToSend) {
                            Log.d(TAG, "ManualBeamInvoke mUrisToSend[" + i + "].toString():"
                                + printUri.toString());
                            i++;
                        }
                    }
                    ...
                    if (mMessageToSend != null) {
                        Log.d(TAG, "ManualBeamInvoke mMessageToSend: "
                            + Util.printNdef(mMessageToSend));
                    }
                } catch (Exception e) {
                    Log.e(TAG, "ManualBeamInvoke Exception:" + e);
                    e.printStackTrace();
                }
                /// }
    ...
    final class SendTask extends AsyncTask<Void, Void, Void> {
        ...
        int doHandover(Uri[] uris, UserHandle userHandle) throws IOException {
            ...
            /// M: @ {
            NdefMessage request;
            request = MtkNfcEntry.getInstance().createRequestMessage(uris);
            if (request == null) {
                request = mHandoverDataParser.createHandoverRequestMessage();
            }
            /// }
            ...
            if (mScenario == MtkNfcEntry.SCENARIO_BEAMPLUS_P2P ||
                ...
                mScenario == MtkNfcEntry.SCENARIO_HR_COLLISION) {
    ...
    boolean allowCertHandover(List<RunningTaskInfo> runningTask) {
        ...
        try {
            ...
            String musicplayActivity = "MusicPlayActivity";

            if (className.endsWith(wpsActivity) || className.endsWith(p2pActivity) || className.endsWith(sigmsActivity) || className.endsWith(musicplayActivity)) {
                result = true;
