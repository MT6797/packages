...
public class NfcService implements DeviceHostListener {
    ...
    MtkNfcEntry.IP2pLinkManager mP2pLinkManager;
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Void> {
        ...
        void printEnableTaskRecord() {
            if (enableTaskRecord.size() != 0) {
                for (Integer recordThread : enableTaskRecord) {
                    Log.d(TAG, "EnableDisableTask !!!! Exception() Thread:"
                        + recordThread + " not finish  !!!!");
                }
        ...
        /// M: @ {
            ...
            try {
                int index = enableTaskRecord.size();
                Log.d(TAG, "EnableDisableTask  add (" + index + "," + threadId + ")");
                enableTaskRecord.add(index, threadId);
            } catch (Exception e) {
                Log.e(TAG, "enableTaskRecord Exception  " + e);
            }
        /// }
            ...
            /// M: @ {
            try {
                int index = enableTaskRecord.size() - 1;
                Log.d(TAG, "EnableDisableTask  remove index-1:" + index);
                enableTaskRecord.remove(index);
            } catch (Exception e) {
                Log.e(TAG, "enableTaskRecord Exception  " + e);
            }
            /// }
    ...
    void saveNfcOnSetting(boolean on) {
        synchronized (NfcService.this) {
            Log.d(TAG, "nfc. PREF_NFC_ON nfc_on to " + on);
    ...
    final class NfcAdapterService extends INfcAdapter.Stub {
        ...
        public boolean disable(boolean saveState) throws RemoteException {
            Log.d(TAG, "disable()   saveState:" + saveState);
            NfcPermissions.enforceAdminPermissions(mContext);
    ...
    class WatchDogThread extends Thread {
        ...
        public WatchDogThread(String threadName, int timeout) {
            ...
            mTimeout = timeout;
            /// M: @ {
            callerThreadId = Process.myTid();
            Log.d(TAG, "Watchdog  callerThreadId:" + callerThreadId);
            /// }
        ...
        public void run() {
            ...
            Log.e(TAG, "Watchdog triggered, aborting. callerThreadId:"
                + callerThreadId + " not finish");
            mDeviceHost.doAbort();
        }
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_SCREEN_ON)
                ...
                Log.d(TAG, "rec. ACTION_AIRPLANE_MODE_CHANGED : " + isAirplaneModeOn);
...
*/
    ...
    List<Integer> enableTaskRecord = new ArrayList<Integer>();
    ...
    private void initAddon() {
        ...
        if (MtkNfcEntry.getInstance().getMode() == MtkNfcEntry.MANAGER_MODE_DTA) {

            mP2pLinkManager = new DtaP2pLinkManagerAdapter(mContext);
    ...
    private final BroadcastReceiver mAddonBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("android.intent.action.ACTION_BOOT_IPO")) {
                ...
                boolean airplaneOverride = mPrefs.getBoolean(PREF_AIRPLANE_OVERRIDE, false);
                boolean airPlaneModeFlag = isAirplaneModeOn();
                boolean perfNfcFlag = mPrefs.getBoolean(PREF_NFC_ON, NFC_ON_DEFAULT);

                Log.d(TAG, "PREF_NFC_ON:" + perfNfcFlag + " airPlaneModeFlag:"
                    + airPlaneModeFlag + "  airplaneOverride:" + airplaneOverride);

//                if (mPrefs.getBoolean(PREF_NFC_ON, NFC_ON_DEFAULT) &&
//                        (!mIsAirplaneSensitive || !isAirplaneModeOn() || airplaneOverride))

                if (perfNfcFlag &&
                    (!mIsAirplaneSensitive || !airPlaneModeFlag || airplaneOverride)) {
                    Log.d(TAG, "IPO -- NFC is on. ");
            ...
            if (action.equals(Intent.ACTION_LOCALE_CHANGED)) {
                Log.d(TAG, "ACTION_LOCALE_CHANGED   mState:" + mState);
                if (isNfcEnabled()) {
                    Log.d(TAG, " NFC is on");

                    Log.d(TAG, " LOCALE_CHANGED hideNotification()");
                    NfcStatusNotificationUi.getInstance().hideNotification();

                    Log.d(TAG, " LOCALE_CHANGED showNotification()");
                    NfcStatusNotificationUi.getInstance().showNotification();
                }
            }
