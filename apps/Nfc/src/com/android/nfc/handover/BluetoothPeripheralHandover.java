...
public class BluetoothPeripheralHandover implements BluetoothProfile.ServiceListener {
    ...
    public BluetoothPeripheralHandover(Context context, BluetoothDevice device, String name,
        ...
        Log.d(TAG, "BluetoothPeripheralHandover() name:" + name + "  transport:" + transport);
        Log.d(TAG, "BluetoothPeripheralHandover() device:" + device);
    ...
    public boolean hasStarted() {
        ...
        Log.d(TAG, "hasStarted()   mState:" + mState);
    ...
    Log.d(TAG, " BluetoothPeripheralHandover.start() ,start 20sec tomeout");
    Log.d(TAG, "     mState:" + mState + "  mTransport:" + mTransport + " mProvisioning:" + mProvisioning + " mBluetoothAdapter:" + mBluetoothAdapter);
    ...
    void nextStepInit() {
        ...
        Log.d(TAG, "nextStepInit() mState:" + mState);
        switch (mState) {
            case STATE_INIT:
    ...
    void nextStepDisconnect() {
        ...
        Log.d(TAG, "nextStepDisconnect() mState:" + mState);
        switch (mState) {
    ...
    boolean getProfileProxys() {
        ...
        if (mTransport == BluetoothDevice.TRANSPORT_LE) {
            ...
            Log.d(TAG, "mBluetoothAdapter.getProfileProxy BluetoothProfile.HEADSET:" + BluetoothProfile.HEADSET);
            if(!mBluetoothAdapter.getProfileProxy(mContext, this, BluetoothProfile.HEADSET))
                return false;

            Log.d(TAG, "mBluetoothAdapter.getProfileProxy BluetoothProfile.A2DP:" + BluetoothProfile.A2DP);
            if(!mBluetoothAdapter.getProfileProxy(mContext, this, BluetoothProfile.A2DP))
                return false;
        }
        Log.d(TAG, "getProfileProxys() , return true");

        return true;
    }

    void nextStepConnect() {
        Log.d(TAG, "nextStepConnect() mState:" + mState);
        ...
        switch (mState) {
            ...
            case STATE_BONDING:
                ...
                Log.d(TAG, " mState to STATE_CONNECTING mTransport:" + mTransport);
                synchronized (mLock) {
                    if (mTransport == BluetoothDevice.TRANSPORT_LE) {
                        if (mInput.getConnectionState(mDevice)
                        ...
                        Log.d(TAG, " BluetoothProfile.STATE_CONNECTED:" + BluetoothProfile.STATE_CONNECTED);

                        int hfConnectionState = mHeadset.getConnectionState(mDevice);
                        Log.d(TAG, " mHeadset.getConnectionState:" + hfConnectionState);
                        if (mHeadset.getConnectionState(mDevice) !=
                                BluetoothProfile.STATE_CONNECTED) {
                            mHfpResult = RESULT_PENDING;

                            Log.d(TAG, " mHeadset.connect");
                            mHeadset.connect(mDevice);
                        } else {
                            mHfpResult = RESULT_CONNECTED;
                        }

                        int a2dpConnectionState = mA2dp.getConnectionState(mDevice);
                        Log.d(TAG, " mA2dp.getConnectionState:" + a2dpConnectionState);
                        if (mA2dp.getConnectionState(mDevice) != BluetoothProfile.STATE_CONNECTED) {
                            mA2dpResult = RESULT_PENDING;

                            Log.d(TAG, " mA2dp.connect");
    ...
    void startBonding() {
        ...
        if (!mDevice.createBond(mTransport)) {
            ...
            Log.d(TAG, "show toast(pairing_peripheral_failed) ");
            complete(false);
        }
    }
    ...
    void handleIntent(Intent intent) {
        ...
        Log.d(TAG, "handleIntent() mState:" + mState + " action:" + action);
        ...
        if (ACTION_ALLOW_CONNECT.equals(action)) {
            ...
            if (bond == BluetoothDevice.BOND_BONDED) {
                ...
                Log.d(TAG, "show toast(pairing_peripheral_failed) 2 ");
                complete(false);
            }
            ...
            Log.d(TAG, "!!!! BluetoothHeadset.ACTION_CONNECTION_STATE_CHANGED  state : " + state + " mState:" + mState);
            ...
            Log.d(TAG, "!!!! BluetoothA2dp.ACTION_CONNECTION_STATE_CHANGED  state : " + state + " mState:" + mState);
            if (state == BluetoothProfile.STATE_CONNECTED) {
                mA2dpResult = RESULT_CONNECTED;
                nextStep();
            } else if (state == BluetoothProfile.STATE_DISCONNECTED) {
    ...
    void complete(boolean connected) {
        if (DBG) Log.d(TAG, "complete()  connected:" + connected);
        ...
        synchronized (mLock) {
            if (mA2dp != null) {
                ...
                Log.d(TAG, "mBluetoothAdapter.closeProfileProxy  BluetoothProfile.A2DP:" + BluetoothProfile.A2DP);
                mBluetoothAdapter.closeProfileProxy(BluetoothProfile.A2DP, mA2dp);
            }
            if (mHeadset != null) {

                Log.d(TAG, "mBluetoothAdapter.closeProfileProxy BluetoothProfile.HEADSET:" + BluetoothProfile.HEADSET);
                mBluetoothAdapter.closeProfileProxy(BluetoothProfile.HEADSET, mHeadset);
            }

            if (mInput != null) {

                Log.d(TAG, "mBluetoothAdapter.closeProfileProxy BluetoothProfile.INPUT_DEVICE:" + BluetoothProfile.INPUT_DEVICE);
