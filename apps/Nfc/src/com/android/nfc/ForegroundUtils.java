...
public class ForegroundUtils extends IProcessObserver.Stub {
    ...
    public List<Integer> getForegroundUids() {
        ...
        Log.d(TAG, " getForegroundUids() mForegroundUidPids.size():" + mForegroundUidPids.size());
        ...
        synchronized (mLock) {
            for (int i = 0; i < mForegroundUidPids.size(); i++) {
                ...
                Log.d(TAG, " mForegroundUidPids.[" + i + "]:" + mForegroundUidPids.keyAt(i));
            }
        }
        return uids;
    }
    ...
    private void handleUidToBackground(int uid) {
        ...
        Log.d(TAG, "  handleUidToBackground  uid:" + uid);
        synchronized (mLock) {
    ...
    public void onForegroundActivitiesChanged(int pid, int uid,
        ...
        synchronized (mLock) {
            ...
            if (hasForegroundActivities) {
               ...
               Log.d(TAG, "  foregroundPids.put(pid, true) " + pid);

            } else {
               foregroundPids.delete(pid);
               Log.d(TAG, "  foregroundPids.delete(pid) " + pid);

            }
            if (foregroundPids.size() == 0) {
                mForegroundUidPids.remove(uid);
                Log.d(TAG, "  mForegroundUidPids.remove  uid:" + uid);

                uidToBackground = true;
            } else {
                mForegroundUidPids.put(uid, foregroundPids);

                Log.d(TAG, "  mForegroundUidPids.put  uid:" + uid);
            }
        }
        if (uidToBackground) {
