...
public class NativeNfcTag implements TagEndpoint {
    ...
    class PresenceCheckWatchdog extends Thread {
        ...
        public synchronized void end() {
            /// M: @ avoid NE when disable nfc with Tag{
            Log.d(TAG, "  PresenceCheckWatchdog.end()");
            /// }
            isStopped = true;
            doCheck = false;
            this.notifyAll();
        ...
        public void run() {
            ...
            /// M: @ avoid NE when disable nfc with Tag{
            Log.d(TAG, "remove synchronized (NativeNfcTag.this)");
    ...
    public synchronized boolean disconnect() {
        ...
        mIsPresent = false;
        /// M: @ avoid NE when disable nfc with Tag{
        Log.d(TAG, " disconnect()  mWatchdog:" + mWatchdog);
        /// }
        if (mWatchdog != null) {
            // Watchdog has already disconnected or will do it
            /// M: @ avoid NE when disable nfc with Tag{
            Log.d(TAG, " disconnect()  mWatchdog.end()");
            /// }
            mWatchdog.end();

            /// M: @ avoid NE when disable nfc with Tag{
            Log.d(TAG, " disconnect() not to call  mWatchdog.join()");
