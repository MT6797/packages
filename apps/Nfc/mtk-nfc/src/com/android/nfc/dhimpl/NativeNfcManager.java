...
/// }
    ...
    //DTA 2 add
    private native boolean doSetPattern(int pattern);

    public boolean setPatternNumber(int pattern) {
        Log.d(TAG, "setPatternNumber() ENTRY pattern:" + pattern);
        boolean result = doSetPattern(pattern);
        Log.d(TAG, "setPatternNumber() EXIT  result:" + result);
        return result;
    }
    ...
    public boolean initialize() {
        ...
        if (sIsUnsupportedChip) {
            Log.d(TAG, "sIsUnsupportedChip==true ,initialize() return false ");
            return false;
        }

        boolean preSetNfcResult = MtkNfcAddonSequence.getInstance().preSetNfc();
        Log.d(TAG, "preSetNfc() result:" + preSetNfcResult);

        boolean patternResult = MtkNfcAddonSequence.getInstance().setDtaPattern();
        Log.d(TAG, "patternResult() result:" + patternResult);
...
*/
    ...
    //small antenna support
    public native boolean doPreSetNfc(int type);

    //DTA 2 add
    private native int doSNL(String sn);

    public int serviceNameLookup(String serviceName) {
        int SAP;

        Log.d(TAG, "ServiceNameLookup doSNL() ENTRY serviceName:" + serviceName);
        SAP = doSNL(serviceName);
        Log.d(TAG, "ServiceNameLookup doSNL() EXIT SAP:" + SAP);
