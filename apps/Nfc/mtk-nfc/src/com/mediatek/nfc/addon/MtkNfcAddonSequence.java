...
public class MtkNfcAddonSequence
    ...
    public static final int MTK_NFC_ANTENNA_TYPE_NORMAL   = 0x00;
    public static final int MTK_NFC_ANTENNA_TYPE_SMALL    = 0x01;
    ...
    private MtkNfcAddonSequence(Context context, NativeNfcManager manager) {
        ...
        mSecureElementSelector = new SecureElementSelector(
                                    mContext, mNativeNfcManager, NfcService.getInstance(), this);
...
*/
    ...
    Settings.Global.putString(
        mContext.getContentResolver(), Settings.Global.NFC_MULTISE_ACTIVE, seString);
    }
    ...
    public void setModeFlag(boolean isNfcEnabled, int mode, int flag, Object syncObj) {
        ...
        if (mode == MODE_READER && mSmallAntennaSupport) {
            Log.d(TAG, "mSmallAntennaSupport == yes, !!!! ignore set MODE_READER command");
            return;
        }
        ...
        synchronized (syncObj) {
            ...
            if (isNfcEnabled) {
                ...
                new ApplyPollingLoopThread(mode, flag
                    , new WatchDogThread("mtk_setmode_applyRouting", 10000), syncObj).start();
            }
        }
    }
    ...
    public void applyInitializeSequence() {
        ...
        Log.d(TAG, "applyInitializeSequence EXIT   isHceCapable:"
                                + isHceCapable + " isHceOn:" + isHceOn);
    ...
    public void applyDeinitializeSequence() {
        ...
        Log.d(TAG, "applyDeinitializeSequence disableCardMode="
            + disableCardMode + "  curMode=" + curMode);
    ...
    public void applyEnableDiscoverySequence() {
        ...
        boolean isHceOn
                = Settings.Global.getInt(mContext.getContentResolver(), "nfc_hce_on", 0) != 0;

        Log.d(TAG, "before check HCE switch, isHceOn: " + isHceOn + "curMode = " + curMode);

        if ((curMode & MODE_CARD) > 0) {
            if (!isHceOn && mSecureElementSelector.getActiveSeValue()
                                                            == SecureElementSelector.USER_OFF) {
                mPrefsEditor.putInt(PREF_MODE_CARD, FLAG_OFF).apply();
    ...
    public boolean preSetNfc() {
        /// precondition: discovery is disabled
        boolean result = false;
        Log.d(TAG, "preSetNfc()");
        if (mSmallAntennaSupport) {
            Log.d(TAG, "mSmallAntennaSupport == yes");
            Log.d(TAG, "doPreSetNfc() ,MTK_NFC_ANTENNA_TYPE_SMALL");
            result = mNativeNfcManager.doPreSetNfc(MTK_NFC_ANTENNA_TYPE_SMALL);
        } else {
            Log.d(TAG, "doPreSetNfc() ,MTK_NFC_ANTENNA_TYPE_NORMAL");
            result = mNativeNfcManager.doPreSetNfc(MTK_NFC_ANTENNA_TYPE_NORMAL);
        }
        Log.d(TAG, "preSetNfc() result:" + result);
        return result;
    }
    ...
    public void storeDtaPatternNumber(int number) {

        Log.d(TAG, "storeDtaPatternNumber() number:" + number);
        mPatternNumber = number;

    }

    public boolean setDtaPattern() {
        boolean result = false;
        Log.d(TAG, "setDtaPattern() mPatternNumber:" + mPatternNumber);
        //int mode = MtkNfcEntry.getInstance().getMode();

        //Log.d(TAG, "MtkNfcEntry.getInstance().getMode():"+mode);
        //if (mode == MtkNfcEntry.MANAGER_MODE_DTA){
        if (mPatternNumber != -1) {
            result = mNativeNfcManager.setPatternNumber(mPatternNumber);
        }

        return result;
    }

    public int serviceNameLookup(String name) {
        int sap;
        sap = mNativeNfcManager.serviceNameLookup(name);
        return sap;
