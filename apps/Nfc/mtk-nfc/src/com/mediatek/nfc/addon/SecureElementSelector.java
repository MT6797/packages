...
public class SecureElementSelector
    ...
    private boolean mIsSwpSim1Retry = false;
    private boolean mIsSwpSim2Retry = false;
    ...
    public boolean handleMessage(Message msg) {
        switch (msg.what) {
            case MSG_TIMEOUT:
                ...
                return true;
            case MSG_SWP_SIM1_RETRY:
                Log.d(TAG, "handleMessage: MSG_SWP_SIM1_RETRY");
                if (!mIsSwpSim1Retry) {
                    mIsSwpSim1Retry = true;
                    Log.d(TAG, "Start retry mechanism for USER_SIM1.");
                    new Thread(new SelectTask(true, USER_SIM1)).start();
                }
                return true;
            case MSG_SWP_SIM2_RETRY:
                Log.d(TAG, "handleMessage: MSG_SWP_SIM2_RETRY");
                if (!mIsSwpSim2Retry) {
                    mIsSwpSim2Retry = true;
                    Log.d(TAG, "Start retry mechanism for USER_SIM2.");
                    new Thread(new SelectTask(true, USER_SIM2)).start();
                }
                return true;
        }
        return false;
    ...
    public SecureElementSelector(Context context, NativeNfcManager nativeNfcManager,
    ...
    private boolean restorePreviousSe() {
        ...
        if (seStr == null) {
            ...
            if (seStr.contains(SETTING_STR_ESE)) {
                seStr = mContext.getString(R.string.mtk_se_ese);
            } else if (seStr.contains(SETTING_STR_SSD)) {
                seStr = mContext.getString(R.string.mtk_se_ssd);
            }

            Log.d(TAG, "no previous record, default = " + seStr);
            mContentProviderUtil.setUserDesiredSeString(seStr);
            mContentProviderUtil.setPreviousActiveSeString(seStr);
        }
    ...
    private class MyContentObserverUtil {
        ...
        private final ContentObserver mObserver = new ContentObserver(null) {
            ...
            public void onChange(boolean selfChange, Uri uri) {
                ...
                /*String activeSe = Settings.Global.getString(mContext.getContentResolver(),
                    NFC_MULTISE_ACTIVE);*/
                Log.d(TAG, "MyContentObserverUtil, onChanged() --> activeSe = " + activeSe +
                            ", selfChange = " + selfChange);
                if (activeSe == null) {
                    Log.d(TAG, "activeSe is still null," +
                                "bypass the ContentObserver onchanged routine!");
                    return;
                }
                if (!mActiveSe.isTheSame(activeSe)) {
                    /**
                     *  Here we use the mActiveSe.isTheSame() to distinguish
                     *  whether this change comes from user.
                     *  The logic is:
                     *      If the change doesn't come from user (for example,
                     *      we cannot find the SE in list during select sequence),
                     *      under this condition, we will sync mActiveSe
                     *      in the end of select sequence.
                     */
    ...
    private class MyContentProviderUtil {
        ...
        public String getActiveSeString() {
            String se = Settings.Global.getString(mContext.getContentResolver(),
                NFC_MULTISE_ACTIVE);
        ...
        public void setPreviousActiveSeString(String prevSeStr) {
            ...
            Settings.Global.putString(mContext.getContentResolver(),
                NFC_MULTISE_PREVIOUS, prevSeStr);
            if (prevSeStr.equals(mContext.getString(R.string.mtk_se_ese))) {
                SharedPreferences spref = mContext.getSharedPreferences(
                    MySecureElement.MYSECUREELEMENT_PREF, Context.MODE_PRIVATE);
                SharedPreferences.Editor editor = spref.edit();
                editor.putString(SETTING_STR_ESE, prevSeStr);
                editor.commit();
            } else if (prevSeStr.equals(mContext.getString(R.string.mtk_se_ssd))) {
                SharedPreferences spref = mContext.getSharedPreferences(
                    MySecureElement.MYSECUREELEMENT_PREF, Context.MODE_PRIVATE);
                SharedPreferences.Editor editor = spref.edit();
                editor.putString(SETTING_STR_SSD, prevSeStr);
                editor.commit();
            }
        }
        ...
        public void setMultiSeSupport(boolean isMultiSeSupport) {
            ...
            Settings.Global.putInt(mContext.getContentResolver(), NFC_MULTISE_ON,
                isMultiSeSupport ? 1 : 0);
        }

        public void setInSwitchingFlag(boolean isInSwitching) {
            Log.d(TAG, "setInSwitchingFlag, isInSwitching = " + isInSwitching);
            Settings.Global.putInt(mContext.getContentResolver(), NFC_MULTISE_IN_SWITCHING,
                isInSwitching ? 1 : 0);
        }

        public void setNfcEnabled(boolean isNfcEnabled) {
            Log.d(TAG, "setNfcEnabled, isNfcEnabled = " + isNfcEnabled);
            Settings.Global.putInt(mContext.getContentResolver(), NFC_MULTISE_NFC_ENABLED,
                isNfcEnabled ? 1 : 0);
        }

        public boolean getNfcEnabled() {
            boolean isNfcEnabled = Settings.Global.getInt(mContext.getContentResolver(),
                NFC_MULTISE_NFC_ENABLED, 0) != 0;
        ...
        public void setUserDesiredSeString(String userDesiredSe) {
            ...
            Settings.Global.putString(mContext.getContentResolver(), NFC_USER_DESIRED_SE,
                userDesiredSe);
        }
        ...
        public void setSeapiSupportCmcc(boolean isSupportCmcc) {
            ...
            Settings.Global.putInt(mContext.getContentResolver(), NFC_SEAPI_SUPPORT_CMCC,
                isSupportCmcc ? 1 : 0);
        }

        public void setSeapiCmccSimNumber(int cmccSimNumber) {
            Log.d(TAG, "setSeapiCmccSimNumber, cmccSimNumber = " + cmccSimNumber);
            Settings.Global.putInt(mContext.getContentResolver(), NFC_SEAPI_CMCC_SIM,
                cmccSimNumber);
        }

        public void setActiveSeNumber(int seNumber) {
            Log.d(TAG, "setActiveSeNumber(), seNumber = " + seNumber);
            Settings.Global.putInt(mContext.getContentResolver(), NFC_MULTISE_ACTIVE_NUM,
                seNumber);
        }
    }
    ...
    private class SelectTask implements Runnable, Handler.Callback {
        ...
        public void run() {
            synchronized (mSyncLock) {
                ...
                if (mIsUpdateSeList) {
                    updateSeMapping();
                    MySecureElement userDesiredSe = MySecureElement.searchByName(
                        mContentProviderUtil.getUserDesiredSeString());
                ...
                try {
                    ...
                    if (!candidateSe.mSettingString.equals(
                        mContentProviderUtil.getActiveSeString())) {
                        needToUpdateDb = true;
                    }
                    ...
                    /**
                     *  Since the "previous active se" is used by Setting
                     *  when it turns card emulation on, the value should never be OFF.
                     *  If user sets a non-Off SE, it should also be the previous active SE.
                     *  If user sets SE to OFF, we use the current active SE
                     *  to be the previous active SE. Otherwise, we set to default.
                     */
                    ...
                    if (duration > MIN_NOTIFY_APP_TIME) {
                        ...
                        mHandler.sendEmptyMessageDelayed(MSG_NOTIFY_APP,
                            MIN_NOTIFY_APP_TIME - duration);
                    }
    ...
    public void checkSettingStrLanguage() {
        ...
        if (str != null) {
            mContentProviderUtil.setActiveSeString(
                MySecureElement.switchStrLanguage(mContext, str));
        }

        str = mContentProviderUtil.getPreviousActiveSeString();
        if (str != null) {
            mContentProviderUtil.setPreviousActiveSeString(
                MySecureElement.switchStrLanguage(mContext, str));
        }
    ...
    private void updateSeMapping() {
        ...
        synchronized (MySecureElement.mSyncSeListLock) {
            ...
            if (mConfigFileParser.get(1, userConfig)) { /// SWP1
                try {
                    ...
                    if (userSe != USER_OFF) {
                        ...
                        if ((userSe == USER_SIM1 && NfcSimStateObserver.getInstance().isSimReady(
                                PhoneConstants.GEMINI_SIM_1) == true) ||
                            (userSe == USER_SIM2 && NfcSimStateObserver.getInstance().isSimReady(
                                PhoneConstants.GEMINI_SIM_2) == true) ||
                            userSe == USER_ESE) {
                    ...
                    if (!mIsSwpSim1Retry && userSe == USER_SIM1 &&
                        NfcSimStateObserver.getInstance().isSimReady(
                            PhoneConstants.GEMINI_SIM_1) == true) {
                        if (!mHandler.hasMessages(MSG_SWP_SIM1_RETRY)) {
                            Log.d(TAG, "Trigger retry mechanism for SWP_SIM1!");
                            mHandler.sendEmptyMessageDelayed(MSG_SWP_SIM1_RETRY, 2000);
                        }
                        Log.d(TAG, "SWP_SIM1 Retry mechanism was triggerd already");
                    }
                    if (!mIsSwpSim2Retry && userSe == USER_SIM2 &&
                        NfcSimStateObserver.getInstance().isSimReady(
                            PhoneConstants.GEMINI_SIM_2) == true) {
                        if (!mHandler.hasMessages(MSG_SWP_SIM2_RETRY)) {
                            Log.d(TAG, "Trigger retry mechanism for SWP_SIM2!");
                            mHandler.sendEmptyMessageDelayed(MSG_SWP_SIM2_RETRY, 2000);
                        }
                        Log.d(TAG, "SWP_SIM2 Retry mechanism was triggerd already");
                    }
                }
            }
            ...
            if (mConfigFileParser.get(3, userConfigSsd) &&
                mConfigFileParser.get(4, userConfigEse)) {
                if (userConfigSsd[0] == 1 && userConfigEse[0] == 1) {
                    Log.e(TAG, "The config is wrong");
                } else {
                    Log.i(TAG, "userConfigSsd[0] = " + userConfigSsd[0] + ", userConfigEse[0] = "
                        + userConfigEse[0]);
            ...
            Log.d(TAG, "isSwpSim1Alarm = " + isSwpSim1Alarm + ", isSwpSim2Alarm = "
                + isSwpSim2Alarm);
    ...
    private void checkAllSwpSimDIalog(boolean isTimeout) {
        ...
        if (!SystemProperties.get("persist.radio.multisim.config").equals("ss")) {
            if (mIsSwpSim1Alarm == true && mIsSwpSim2Alarm == true) {
                if (isTimeout == false) {
                    if (NfcSimStateObserver.getInstance().isSimReady(
                            PhoneConstants.GEMINI_SIM_1) == false ||
                        NfcSimStateObserver.getInstance().isSimReady(
                            PhoneConstants.GEMINI_SIM_2) == false) {
    ...
    private void checkSingleSimDialog() {
        Log.i(TAG, "checkSingleSimDialog mIsSwpSim1Alarm = " + mIsSwpSim1Alarm +
            " mIsSwpSim2Alarm = " + mIsSwpSim2Alarm);
    ...
    private void onSeRefresh(int updatedSe, int event) {
        ...
        if (NfcSimStateObserver.SimEventListener.NOT_READY == event && USER_SIM1 == updatedSe) {
            Log.d(TAG, "USER_SIM1 is not ready, reset SWP_SIM1 retry mechanism.");
            mIsSwpSim1Retry = false;
        } else if (NfcSimStateObserver.SimEventListener.NOT_READY == event &&
            USER_SIM2 == updatedSe) {
            Log.d(TAG, "USER_SIM2 is not ready, reset SWP_SIM2 retry mechanism.");
    ...
    private void notifyApplication(int type, Object obj) {
        ...
        if (type == NOTIFY_SELECTION_FAIL) {
            ...
            if (NfcRuntimeOptions.isSupportNonNfcSimPopup() &&
                mShouldSimWarningDialogPopup[(type == NOTIFY_SIM1_NOT_NFC ? 0 : 1)]) {
    ...
    private void showAlternativeSeDialog(String originalSe) {
         ...
         * for the temporary state that the se is not exist.*/
            ...
            message = originalSe + mContext.getString(R.string.mtk_se_alt) +
                MySecureElement.getFirstSe().mSettingString + "?";
    ...
    static class MySecureElement {
        ...
        static private String SETTING_STRs[] = {SETTING_STR_OFF, SETTING_STR_SIM1, SETTING_STR_SIM2
                                                , SETTING_STR_SSD, SETTING_STR_ESE};
        static public Object mSyncSeListLock = new Object();
        static private final String MYSECUREELEMENT_PREF = "MySecureElementPrefs";

        static public String switchStrLanguage(Context context, String str) {
            SharedPreferences spref =
                context.getSharedPreferences(MYSECUREELEMENT_PREF, Context.MODE_PRIVATE);
            if (str.contains(SETTING_STRs[USER_SSD]) ||
                str.contains(spref.getString(SETTING_STR_SSD, SETTING_STR_SSD))) {
                Log.d(TAG, "MySecureElement.switchSettingStrLanguage(), " + str + " switch to "
                    + context.getString(R.string.mtk_se_ssd));
                str = context.getString(R.string.mtk_se_ssd);
            } else if (str.contains(SETTING_STRs[USER_ESE]) ||
                str.contains(spref.getString(SETTING_STR_ESE, SETTING_STR_ESE))) {
                Log.d(TAG, "MySecureElement.switchSettingStrLanguage(), " + str + " switch to "
                    + context.getString(R.string.mtk_se_ese));
        ...
        static public MySecureElement searchByName(String seStr, boolean exactlyMatch,
                        boolean selectOffWhenMiss) {
        ...
        static public MySecureElement getFirstSe() {
            MySecureElement firstSE;
            synchronized (mSyncSeListLock) {
                firstSE = sSeList.get(0);
            }
            return firstSE;
        }
        ...
        private MySecureElement(int userSe, int chipSe, Context context, boolean isBundleSimState)
        {
            ...
            Log.d(TAG, "new MySecureElement: userSe = " + mUserSe + ", chipSe = " + mChipSe
                + ", string = " + mSettingString);
        }
        ...
        static public String getSettingString(int userSe, Context context) {
            if (userSe == USER_OFF || userSe == USER_SSD || userSe == USER_ESE) {
                ...
                try {
                    /*ITelephony manager = ITelephony.Stub.asInterface(
