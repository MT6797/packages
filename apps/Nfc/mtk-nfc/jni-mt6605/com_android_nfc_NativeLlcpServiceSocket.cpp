...
extern int gDta2PatternNo;

#define LLCP_EXTEND_LENGTH 128
...
static jobject com_NativeLlcpServiceSocket_doAccept(JNIEnv *e, jobject o, jint miu, jint rw, jint linearBufferLength)
{
   ...
   if (hServerSocket != 0)
   {
       ...
       Accept.buffer_options.miu = miu;//LLCP_EXTEND_LENGTH;//miu;
       ...
       if (CurrentIdx < NFC_LLCP_SERVICE_NUM)
       {
          ...
          if(gDta2PatternNo == -1)
          {
              ...
              hIncomingSocket = getIncomingSocket(pMonitor, hServerSocket,&Needclose);
          }
          else
          {
          while(1)
          ...
          pthread_cond_wait(&pMonitor->incoming_socket_cond[CurrentIdx], &pMonitor->incoming_socket_mutex[CurrentIdx]);
            ...
            if((hIncomingSocket)||(Needclose))
            {
                break;
            }
            else
            {
                ALOGD("pthread_cond_wait false alarm");
            }
          ...
          if (hIncomingSocket != 0)
          {
              ...
              /* Accept the incomming socket */
               TRACE("com_NativeLlcpServiceSocket_doAccept,miu,%d,rw,%d,idx%d,H,0x%x", Accept.buffer_options.miu ,Accept.buffer_options.rw, CurrentIdx,Accept.incoming_handle);
           ...
           /* Create the local semaphore */
               ...
               ALOGD("com_NativeLlcpServiceSocket_doAccept() returned 0x%04x,idx %d", ret, CurrentIdx);
       ...
       if (CurrentIdx < NFC_LLCP_SERVICE_NUM)
       {
...

           {
                ...
                else
                {
                    ...
                    pListenData->pIncomingSocket = hLlcpSocket;
                    pListenData->NeedClose = 1;
