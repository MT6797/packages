...
void nfc_jni_llcp_receive_callback(int  status, unsigned char sap, unsigned char* Buf, unsigned int Length, unsigned int service)
{
   ...
   ALOGD("nfc_jni_llcp_doReceive_callback,idx%d,sta%d,len%d,ser0x%x",idx, status, Length, service);
   ...
   if ( (idx < NFC_LLCP_SERVICE_NUM ) && (g_llcp_cb_data[idx][NFC_LLCP_RECEIVE] != NULL))
   {
       ...
       ALOGD("nfc_jni_llcp_doReceive_callback,OK,idx%d", idx);
...
static jboolean com_android_nfc_NativeLlcpSocket_doClose(JNIEnv *e, jobject o)
{
   ...
   if (hLlcpSocket != 0)
   {
       ...
       if (CurrentIdx < NFC_LLCP_SERVICE_NUM)
       {
           ...
           ret = android_nfc_jni_send_msg(MTK_NFC_P2P_DISC_CLIENT_REQ, TotalLength, (void *)&hLlcpSocket);
           if (ret == FALSE)
           {
...
static jboolean com_android_nfc_NativeLlcpSocket_doSend(JNIEnv *e, jobject o, jbyteArray  data)
{
   ...
   if ((hLlcpSocket != 0) && (device_connected_flag == 1))
   {
       ...
       ALOGD("com_android_nfc_NativeLlcpSocket_doSend,idx:%d,0x%x,0x%x", CurrentIdx, g_llcp_service[CurrentIdx], hLlcpSocket);
       ...
       if (CurrentIdx < NFC_LLCP_SERVICE_NUM)
       {
            ...
            ALOGD("com_android_nfc_NativeLlcpSocket_doSend returned,idx:%d,0x%04x,ret:%d",CurrentIdx, hLlcpSocket, ret);
...
static jint com_android_nfc_NativeLlcpSocket_doReceive(JNIEnv *e, jobject o, jbyteArray  buffer)
{
   ...
   if ((hLlcpSocket != 0)&&(device_connected_flag ==1))
   {
       ...
       ALOGD("com_android_nfc_NativeLlcpSocket_doReceive,idx:%d,0x%x,0x%x", CurrentIdx, g_llcp_service[CurrentIdx], hLlcpSocket);
       ...
       if (CurrentIdx < NFC_LLCP_SERVICE_NUM)
       {
           ...
           ALOGD("com_android_nfc_NativeLlcpSocket_doReceive max rcv len,%d", sReceiveLength[CurrentIdx]);
          ...
          // MTK_NFC_P2P_RECV_DATA_REQ
           ...
           //if((ret == 0) && (sReceiveLength[CurrentIdx] != 0)) // dta2 , need echo zero-length packet @@
           if(ret == 0)
           {
              char* DbgBuf = NULL;
              unsigned int Index = 0, DbgLth = 0;
           ...
           ALOGD("com_android_nfc_NativeLlcpSocket_doReceive returned,idx%d,h:0x%04x,ret:%d",CurrentIdx, hLlcpSocket, result);
        clean_and_return:
