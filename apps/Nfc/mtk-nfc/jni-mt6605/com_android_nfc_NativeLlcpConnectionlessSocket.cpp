...
extern int     sem_wait_count;//for semaphore debug
extern int     sem_post_count;//for semaphore debug


namespace android {

extern void android_nfc_print_debuglog_array(UINT8* data, UINT32 length);
/*
 * Callbacks
 */
...
static jboolean com_android_nfc_NativeLlcpConnectionlessSocket_doSendTo(JNIEnv *e, jobject o, jint nsap, jbyteArray data)
{
   ...
   if ((hLlcpSocket != 0)&&(device_connected_flag == 1))
   {
       ...
       ALOGD("com_android_nfc_NativeLlcpConnectionlessSocket_doSendTo,idx,%d,handle,0x%x,0x%x", g_idx_service[NFC_LLCP_SEND], g_llcp_service[g_idx_service[NFC_LLCP_SEND]], hLlcpSocket);
       ...
       if (g_idx_service[NFC_LLCP_SEND] < NFC_LLCP_SERVICE_NUM)
       {
           ...
           ALOGD("com_android_nfc_NativeLlcpConnectionlessSocket_doSendTo(),idx:%d , SAP:0x%x",idx,pData->sap);
...
static jobject com_android_nfc_NativeLlcpConnectionlessSocket_doReceiveFrom(JNIEnv *e, jobject o, jint linkMiu)
{
   ...
   if ((hLlcpSocket != 0)&&(device_connected_flag == 1))
   {
       ...
       ALOGD("com_android_nfc_NativeLlcpConnectionlessSocket_doReceiveFrom ,idx,%d,handle,0x%x,0x%x", CurrentIdx, g_llcp_service[CurrentIdx], hLlcpSocket);
       ...
       if (CurrentIdx < NFC_LLCP_SERVICE_NUM)
       {
           ...
           sReceiveBuffer[CurrentIdx] = (unsigned char*)malloc(500);
           ALOGE("New buf %p , %p\n",g_llcp_cb_data[CurrentIdx][NFC_LLCP_RECEIVE]->pContext,sReceiveBuffer[CurrentIdx]);
...
static jboolean com_android_nfc_NativeLlcpConnectionlessSocket_doClose(JNIEnv *e, jobject o)
{
   ...
   if (hLlcpSocket != 0)
   {
       ...
       ALOGD("com_android_nfc_NativeLlcpConnectionlessSocket_doClose idx,%d,handle,0x%x,0x%x", CurrentIdx , g_llcp_service[CurrentIdx], hLlcpSocket);
       ...
       if (CurrentIdx < NFC_LLCP_SERVICE_NUM)
       {
           ...
           ALOGD("com_android_nfc_NativeLlcpConnectionlessSocket_doClose idx%d ,ret0x%04x", CurrentIdx,ret);
        clean_and_return:
           nfc_cb_data_deinit(&cb_data);
