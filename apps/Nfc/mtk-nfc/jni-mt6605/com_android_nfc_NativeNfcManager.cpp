...
// NFC antenna type
#define MTK_NFC_ANTENNA_TYPE_NORMAL 0x00
#define MTK_NFC_ANTENNA_TYPE_SMALL  0x01
...
int     gTimeout                = 1000;
int     gAntennaType            = MTK_NFC_ANTENNA_TYPE_NORMAL;
int     sem_wait_count                 =0;//for semaphore debug
int     sem_post_count                 =0;//for semaphore debug
...
//DTA2
//DTA2 pattern number
int     gDta2PatternNo = -1;

#define DTA2_LLCP_PATTERN_MASK      0x0000FFFF
#define DTA2_LLCP_TYPE_MASK         0x00030000
#define DTA2_LLCP_EXTERNDED_MASK    0xFFFC0000

#define GET_DTA2_EXT_PAT_NUM(pat) (((pat)& DTA2_LLCP_EXTERNDED_MASK) >> 18)

#define DTA2_LLCP_EPAT_MIUX_EXTERD     0x01
#define DTA2_LLCP_EPAT_RW_0     0x03
#define DTA2_LLCP_EPAT_RW_2     0x04
//----------------------------------
//  DTA
//----------------------------------
#ifdef DTA_SUPPORT
...
static int android_nfc_socket_init(void)
{
    ...
    // connect to server - max retry 10 times
...
#define MAX_SOCKET_FAIL_RETRY_CNT 10
    ...
    while(nat->running == TRUE)
    {
        ...
        switch(prmsg->type)
        {
            ...
            case MTK_NFC_JNI_P2P_SNL_RES:
            {
                mtk_nfc_SNL_res_t* pSNL_Res;
                pSNL_Res = (mtk_nfc_SNL_res_t*)((unsigned char*)prmsg+sizeof(MTK_NFC_MSG_T));

                ALOGD("MTK_NFC_JNI_P2P_SNL_RES,  SAP%d\n", pSNL_Res->u1Sap);

                if (g_working_cb_data != NULL)
                {
                    g_working_cb_data->status = pSNL_Res->u4Result;
                    nat->SAP= pSNL_Res->u1Sap;
                    sem_post(&g_working_cb_data->sem);
                    sem_post_count++;
                    ALOGD("%s: sem_post_count =  %d", __FUNCTION__, sem_post_count);
                }
...
static int nfc_jni_initialize(struct nfc_jni_native_data *nat)
{
    ...
    ALOGD("%s:dta_mode = %d",__FUNCTION__,dta_mode);
    #endif
    ...
    sem_wait_count++;
    ALOGD("%s: sem_wait_count =  %d", __FUNCTION__, sem_wait_count);
...
//              return TRUE;
    ...
    {
        ALOGD("%s,DTA flow !",__FUNCTION__);
        //default normal mode ,clear flag
        dta_nfc_jni_set_dta_quick_mode(FALSE);
        dta_quick_mode=FALSE;
        quick_enable_mode=FALSE;
    }
    ...
    s_mtk_nfc_service_activate_param_t rActivateParam)
{
     ...
     || (rActivateParam.rRfTechParam.u1Mode == NFC_DEV_DISCOVERY_TYPE_POLL_F_ACTIVE))
    {
        ...
        GenArray = rActivateParam.rIntfaceParam.InterfaceParam.rPFNfcDep.au1GenBytes;
        GenLen = rActivateParam.rIntfaceParam.InterfaceParam.rPFNfcDep.u1GenBytesLen;
    }
    else
    {
        ALOGD("Discovered P2P Initiator");
        e->SetIntField(tag, f, (jint)MODE_P2P_INITIATOR);
        GenArray = rActivateParam.rIntfaceParam.InterfaceParam.rLFNfcDep.au1GenBytes;
        GenLen = rActivateParam.rIntfaceParam.InterfaceParam.rLFNfcDep.u1GenBytesLen;
    }
    //android_nfc_print_debuglog_array(GenArray,GenLen);

    /*Set mLlcpVersion*/
    //get llcp version...
    u1LlcpVer = 0x10;
    if((GenLen > 6) && (GenArray[0] == 0x46)&& (GenArray[1] == 0x66)&& (GenArray[2] == 0x6D))
    {
        ParsingIndex = 3;//skip magic number...
        while(ParsingIndex < GenLen)
        {
            u1ParamId = GenArray[ParsingIndex++];
            u1ParamLen = GenArray[ParsingIndex++];
            if(u1ParamId == 0x01)//llcp version
            {
                u1LlcpVer = GenArray[ParsingIndex];
            }
            ParsingIndex += u1ParamLen;
        }
    }
    TRACE("mLlcpVersion = 0x%x",u1LlcpVer);

#if 0   //for M version CTS , workaround , force LLCP higher than 1.2
    if(u1LlcpVer < 0x12)
    {
        u1LlcpVer = 0x12;
        TRACE("for M workaround , mLlcpVersion = 0x%x",u1LlcpVer);
    }
#endif

    f = e->GetFieldID(tag_cls, "mLlcpVersion", "B");
    e->SetByteField(tag, f,(jint)u1LlcpVer);

    /*Set mGeneralBytes*/
    f = e->GetFieldID(tag_cls, "mGeneralBytes", "[B");
    {
        jbyteArray generalBytes = NULL;
        generalBytes = e->NewByteArray (GenLen);

        e->SetByteArrayRegion(generalBytes, 0, GenLen, (jbyte *)GenArray);
        e->SetObjectField(tag, f, generalBytes);

        if(generalBytes != NULL)
        {
           e->DeleteLocalRef(generalBytes);
        }
    }
...
static void nfc_jni_llcp_linkStatus_callback(struct nfc_jni_native_data *Natdata, int status, unsigned char LinkStatus)
{
    ...
    ALOGD("Callback: nfc_jni_llcp_linkStatus_callback(),%p,linkstatus,status", nat);
    if (nat != NULL)
    {
        nat->vm->AttachCurrentThread (&e, NULL);
        /* Update link status */
...
static jboolean com_android_nfc_NfcManager_initialize(JNIEnv *e, jobject o)
{
    ...
    if(gDta2PatternNo != -1)
    {
        ...
        nat->rDiscoverConfig.u1DtaTestMode = 0xFF; //not DTA
        nat->rDiscoverConfig.i4DtaPatternNum = gDta2PatternNo;


        switch(gDta2PatternNo & DTA2_LLCP_TYPE_MASK)
        {
        case 0x0:
            nat->rDiscoverConfig.p2p_setting.u1BitRateF = 2; // 212 // restore
        case 0x10000:
            nat->rDiscoverConfig.p2p_setting.u1BitRateF = 2; // 212
            break;
        case 0x20000:
            nat->rDiscoverConfig.p2p_setting.u1BitRateF = 4; // 424
            break;
        default:
            nat->rDiscoverConfig.p2p_setting.u1BitRateF = 2; // 212 // restore
            break;
...
static jintArray com_android_nfc_NfcManager_doGetSecureElementList(JNIEnv *e, jobject o) {
    ...
    ///SWP conformance in normal mode
    {
        ...
        else
        {
            ...
            if(rv < 1)
            {
               ALOGD("%s: err read data.", __FUNCTION__);
            }
            else
            {
...
#if 1
static jint com_android_nfc_NfcManager_doLlcpServiceNameLookup(JNIEnv *e, jobject o, jstring sn)
{
    jint LookupResultSAP = -1;
#if 1
    ...
    mtk_nfc_SNL_req_t* pSNL = NULL;
    NFCSTATUS ret;

     /* Retrieve native structure address */
    nat = nfc_jni_get_nat(e, o);

    //check llcp has activated
    TRACE("com_android_nfc_NfcManager_doLlcpServiceNameLookup,u1DevActivated(%d),u1Protocol",nat->u1DevActivated);
    if(nat->u1DevActivated == 2)
    {
        if(nat->rDevActivateParam.u1Protocol== NFC_DEV_PROTOCOL_NFC_DEP)
        {
            //OK
        }
        else
        {
            return LookupResultSAP;
        }
    }
    else
    {
   ...
   //-------------------------------------------------------
    ...
    {
        TotalLength = sizeof(mtk_nfc_SNL_req_t) + ServiceNameLgh;
        pSNL = (mtk_nfc_SNL_req_t*)malloc(TotalLength);

        if(pSNL)
        {
            memcpy(pSNL->llcp_sn.buffer, ServiceNameBuf, ServiceNameLgh);
            pSNL->llcp_sn.length = ServiceNameLgh;
            android_nfc_print_debuglog_array(ServiceNameBuf,ServiceNameLgh);
        }
        else
        {
            ALOGE("fail 1\n");
    ...
    ret = android_nfc_jni_send_msg(MTK_NFC_JNI_P2P_SNL_REQ, TotalLength, (void *)pSNL);
...
#if 1
static jboolean com_android_nfc_NfcManager_doSetPatternNumber(JNIEnv *e, jobject o, jint pattern_num)
{
    jboolean ret = TRUE;
    struct nfc_jni_native_data *nat;

    TRACE("com_android_nfc_NfcManager_doSetPatternNumber Enter patternNo 0x%x", pattern_num);

    gDta2PatternNo = pattern_num;
        ...
        jint nSap, jstring sn)
{
   ...
   TRACE("com_android_nfc_NfcManager_doCreateLlcpConnectionlessSocket,pattern No 0x%x", gDta2PatternNo);
   //if((gDta2PatternNo == -1)||((gDta2PatternNo &0x00FC0000) == 0x00040000))
   if(gDta2PatternNo == -1)
   {
        MIUValue = LLCP_DEFAULT_LENGTH;
   }
   else if(GET_DTA2_EXT_PAT_NUM(gDta2PatternNo) == DTA2_LLCP_EPAT_MIUX_EXTERD)
   {

        MIUValue = LLCP_EXTEND_LENGTH;
   }
   else
   {
        MIUValue = LLCP_DEFAULT_LENGTH;
   ...
   pSocket->buffer_options.miu = MIUValue;
   ...
   if(nSap < 0x20)// how to know this connectionless is server or client
   {
   ...
   e->SetIntField(connectionlessSocket, f,(jint)MIUValue);
...
static jobject com_android_nfc_NfcManager_doCreateLlcpServiceSocket(JNIEnv *e, jobject o, jint nSap, jstring sn, jint miu, jint rw, jint linearBufferLength)
{
    ...
    TRACE("com_android_nfc_NfcManager_doCreateLlcpServiceSocket,pattern No 0x%x", gDta2PatternNo);
    if(gDta2PatternNo == -1)
    {
        MIUValue = miu;
    }
    ...
    else if(GET_DTA2_EXT_PAT_NUM(gDta2PatternNo) == DTA2_LLCP_EPAT_RW_0)
    {
        rw = 0;
    }
    else if(GET_DTA2_EXT_PAT_NUM(gDta2PatternNo) == DTA2_LLCP_EPAT_RW_2)
    {
        rw = 2;
    }
    else
    ...
    pSocket->buffer_options.miu = MIUValue;//255;// miu;
...
static jobject com_android_nfc_NfcManager_doCreateLlcpSocket(JNIEnv *e, jobject o, jint nSap, jint miu, jint rw, jint linearBufferLength)
{
    ...
    // MTK_NFC_LLCP_RSP_SERVICE* pSerRsp = NULL;
    TRACE("com_android_nfc_NfcManager_doCreateLlcpSocket,pattern No 0x%x", gDta2PatternNo);
    if(gDta2PatternNo == -1)
    {
        MIUValue = miu;
    }
    ...
    pSocket->buffer_options.miu = MIUValue;//miu;
    pSocket->buffer_options.rw = rw;
    pSocket->connection_type = MTK_NFC_LLCP_CONNECTION_ORIENTED;
...
static jboolean com_android_nfc_NfcManager_doPreSetNfc(JNIEnv *e, jobject o,
    jint type)
{
    jboolean result = JNI_TRUE;
    ...
    ALOGD("com_android_nfc_NfcManager_doPreSetNfc, type %d, gAntennaType, %d", type, gAntennaType);
...
static JNINativeMethod gMethods[] =
{
   ...
   //********** MTK Customized - start **********//
    ...
    {"doPreSetNfc", "(I)Z",
        (void*) com_android_nfc_NfcManager_doPreSetNfc},

#if 1 /*DTA2 add */
    {"doSNL", "(Ljava/lang/String;)I",
       (void *)com_android_nfc_NfcManager_doLlcpServiceNameLookup},

    {"doSetPattern", "(I)Z",
       (void *)com_android_nfc_NfcManager_doSetPatternNumber},
#endif
