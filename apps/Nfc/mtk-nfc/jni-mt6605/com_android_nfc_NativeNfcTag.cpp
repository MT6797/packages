   ...
   jobject o)
{
   ...
   if (nfc_cb_data_init(&cb_data, NULL))
   {
      ...
      if (result == FALSE)
      {
            ALOGE("send MTK_NFC_JNI_TAG_READ_REQ fail\n");
   ...
   jobject o, jbyteArray buf)
{
   ...
   if (nfc_cb_data_init(&cb_data, NULL))
   {
      ...
      if (result == FALSE)
      {
            ALOGE("send MTK_NFC_JNI_TAG_WRITE_REQ fail\n");
            status = JNI_FALSE;
            if(p_data) free(p_data);
            goto error;
      }
...
static jint com_android_nfc_NativeNfcTag_doCheckNdef(JNIEnv *e, jobject o, jintArray ndefinfo)
{
   ...
   if (nfc_cb_data_init(&cb_data, NULL))
   {
      ...
      if (result == FALSE)
      {
            ALOGE("send MTK_NFC_JNI_CHECK_NDEF_REQ fail\n");
            goto error;
      }
...
static jboolean com_android_nfc_NativeNfcTag_doNdefFormat(JNIEnv *e, jobject o, jbyteArray key)
{
   ...
   if (nfc_cb_data_init(&cb_data, NULL))
   {
      ...
      if (result == FALSE)
      {
            ALOGE("send MTK_NFC_JNI_TAG_FORMAT_REQ fail\n");
            goto error;
      }
