...
//Not required
    ...
    private void setAlbumArtBackground() {
        ...
        MusicLogUtils.i(TAG, "setAlbumArtBackground: mAlbumId = "
    ...
    private TouchInterceptor.DropListener mDropListener =
        ...
        public void drop(int from, int to) {
            if (mTrackCursor instanceof NowPlayingCursor) {
                ...
                MusicLogUtils.d(TAG, "drop: from = " + from + ", to = " + to +
    ...
    public boolean onContextItemSelected(MenuItem item) {
     ...
     private void showCreateDialog(String plst_item_id,
                                   ...
                                   String plst_flag,
                                   long [] list1) {
        MusicLogUtils.i(TAG, "showCreateDialog>>");
        removeOldFragmentByTag(DIALOG_TAG_CREATE);
        FragmentManager fragmentManager = getFragmentManager();
        MusicLogUtils.i(TAG, "<showCreateDialog> fragmentManager = " + fragmentManager);
    ...
    private void showDeleteDialog(Long track_id, int str_id, String track_name) {
        MusicLogUtils.i(TAG, "showDeleteDialog>>");
        removeOldFragmentByTag(DIALOG_TAG_DELETE);
        FragmentManager fragmentManager = getFragmentManager();
        MusicLogUtils.i(TAG, "<showDeleteDialog> fragmentManager = " + fragmentManager);
   ...
   private void removeOldFragmentByTag(String tag) {
        MusicLogUtils.i(TAG, "<removeOldFragmentByTag> tag = " + tag);
        FragmentManager fragmentManager = getFragmentManager();
        MusicLogUtils.i(TAG, "<removeOldFragmentByTag> fragmentManager = " + fragmentManager);
        DialogFragment oldFragment = (DialogFragment) fragmentManager.findFragmentByTag(tag);
        MusicLogUtils.i(TAG, "<removeOldFragmentByTag> oldFragment = " + oldFragment);
        if (null != oldFragment) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            default:
                Bundle options = new Bundle();
                options.putString(PluginUtils.PLAYLIST_NAME, mPlaylist);
                options.putInt(PluginUtils.PLAYLIST_LEN, (mTrackList == null) ?
    ...
    static class TrackListAdapter extends SimpleCursorAdapter implements SectionIndexer {
        ...
        class TrackQueryHandler extends AsyncQueryHandler {
            ...
            protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
                MusicLogUtils.i(TAG, "query complete: count is "
        ...
        private void getColumnIndices(Cursor cursor) {
            if (cursor != null) {
                ...
                /// M: add for chinese sorting
                mTitlePinyinIdx =
                 cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.TITLE_PINYIN_KEY);
