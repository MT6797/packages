...
//		};
	...
	private ArrayList<OnIdentifyRspCB> mIdentifyListeners = new ArrayList<OnIdentifyRspCB>();
...
//		}
	...
	public void addIdentifyListener(OnIdentifyRspCB callback) {
	    Log.d(TAG,"addIdentifyListener   OnIdentifyRspCB: " + callback);
	    mIdentifyListeners.add(callback);
    }
	
	public void removeIdentifyListener(OnIdentifyRspCB callback) {
	    Log.d(TAG,"removeIdentifyListener   OnIdentifyRspCB: " + callback);
	    if(mIdentifyListeners.contains(callback)) {
	        mIdentifyListeners.remove(callback);
	    }
    }
	...
	private void identifyCredentialRSP(int index, int result, int fingerid) {
		Log.d(TAG,"identifyCredentialRSP $$$$$$$ index = "+index+"result :"+result+"fingerid :"+fingerid + " , " + mIdentifyListeners);
		for(OnIdentifyRspCB listener : mIdentifyListeners) {
		    Log.d(TAG,"identifyCredentialRSP $$$$$$$ listener = "+listener);
		    if (listener != null) {
		        listener.onIdentifyRsp(index, result,fingerid);
		    }
		}
        if (result == ENROLL_CANCLED) {
            mCanceling = false;
