...
public class SapMessage {
    ...
    private boolean parseParameters(int count, InputStream is) throws IOException {
        ...
        for(int i = 0; i < count; i++) {
            ...
            /// M: ALPS02343495, Correct the calculation of padding byte length @{
            if ((paramLength % 4) != 0) {
    ...
    private static void writeParameter(OutputStream os, int id, byte[] value) throws IOException {
        ...
        /* Payload */
        os.write(value);
        /// M: Correct the calculation of padding byte length @{
        if ((value.length % 4) != 0) {
...
//            break;
    ...
    private void createSolicited(MsgHeader msg) throws IOException,
        ...
        case ID_CONNECT_REQ:
        {
            ...
            // M : MaxMsgSize in only used with "Error, Server does not support message size"
            if (mConnectionStatus != RIL_SIM_SAP_CONNECT_RSP.RIL_E_SAP_MSG_SIZE_TOO_LARGE) {
                mMaxMsgSize = INVALID_VALUE;
            }
