...
public class GattService extends ProfileService {
    ...
    private boolean testContinueServiceDeclarationStartAndSet() {
        ...
        Log.d(TAG, "testContinueServiceDeclarationStartAndSet: bStart=" + bStart);
        return bStart;
    }
        ...
        for (ScanClient client : mScanManager.getRegularScanQueue()) {
            if (client.uuids.length > 0) {
                ...
                for (UUID search : client.uuids) {
                    Log.d(TAG, "client.uuid = " + search);
                    for (UUID remote: remoteUuids) {
                        if (remote.equals(search)) {
                            Log.d(TAG, "Match uuid = " + search);
                            ++matches;
    ...
    void startScan(int appIf, boolean isServer, ScanSettings settings,
        ...
        /// M: ALPS02073379: This log is not appropriate, we add below with a statement
        ...
        if (filters != null && filters.size() > 0) {
            if (DBG) Log.d(TAG, "start scan with filters");
            if (DBG) Log.d(TAG, "filter List = " + filters.toString());
        } else {
            if (DBG) Log.d(TAG, "start scan without filters");
        }
    ...
    void clientConnect(int clientIf, String address, boolean isDirect, int transport) {
        ...
        if (DBG) Log.d(TAG, "clientConnect() - clientIf =" + clientIf + ", address=" + address
                + ", isDirect=" + isDirect + ", transport = " + transport);
    ...
    void startMultiAdvertising(int clientIf, AdvertiseData advertiseData,
            AdvertiseData scanResponse, AdvertiseSettings settings) {
        enforceAdminPermission();
        Log.d(TAG, "startMultiAdvertising, clientIf = " + clientIf);
        if (DBG && advertiseData != null) {
            Log.d(TAG, "advertiseData = " + advertiseData.toString());
        }
        if (DBG && scanResponse != null) {
            Log.d(TAG, "scanResponseData = " + scanResponse.toString());
        }
        if (DBG && settings != null) {
            Log.d(TAG, "advertiseSettings = " + settings.toString());
        }
    ...
    void stopMultiAdvertising(AdvertiseClient client) {
        enforceAdminPermission();
        Log.d(TAG, "stopMultiAdvertising, clientIf = " + client.clientIf);
    ...
    void onClientConnected(String address, boolean connected, int connId, int serverIf)
        ...
        if (DBG) Log.d(TAG, "onConnected() connId=" + connId
            + ", address=" + address + ", connected=" + connected + ", serverIf=" + serverIf);
    ...
    void endServiceDeclaration(int serverIf) {
        ...
        if (DBG) Log.d(TAG, "endServiceDeclaration() - serverIf=" + serverIf + 
                            ", getActiveDeclaration().mServerIf=" + getActiveDeclaration().mServerIf);

        /// M: ALPS02058456: Fix add-service concurrency issue.
        if (serverIf != getActiveDeclaration().mServerIf) {
            getLastestDeclaration(serverIf).endServiceDeclaration();
        } else {
            getActiveDeclaration().endServiceDeclaration();
        }
        /// @}
    ...
    private void continueSearch(int connId, int status) throws RemoteException {
        ...
        if (status == 0 && !mSearchQueue.isEmpty(connId)) {
            ...
            if (svc == null) {
                Log.e(TAG, "continueSearch() - no entry of  connId=" + connId);
                return;
            }
    ...
    private void continueServiceDeclaration(int serverIf, int status, int srvcHandle) throws RemoteException {
        ...
        if (entry != null) {
            ...
            /// M: ALPS02058456: Fix add-service concurrency issue.
            if (getPendingDeclaration().isEndServiceDeclaration()) {
                ...
                /// M: ALPS02058456: Fix add-service concurrency issue.
                if (DBG) Log.d(TAG, "continueServiceDeclaration() - wait endServiceDeclaration, serverIf=" + serverIf);
                resetContinueServiceDeclarationStart();
                /// @}
            }
        }
