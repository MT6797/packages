...
public class BluetoothMapUtils {
    ...
    private static final String TAG = "[MAP]BluetoothMapUtils";
     ...
     */
    public static void writeHeader(StringBuilder writer, String name, String value)
      throws IOException {
     ...
     * @param value the header value
     */
    public static void writeEncodedHeader(StringBuilder writer, String name, String value)
            throws IOException {
        if (value != null && value.length() > 0) {
            writer.append(name);
            writer.append(": ");
        ...
        //    writer.append(MimeUtility.foldAndEncode2(value, name.length() + 2));
            writer.append("=?");
            writer.append("UTF-8?Q?");
            writer.append(encodeQForm(value));
            writer.append("?=");
            writer.append("\r\n");
        }
    }
    public static void writeAddressHeader(StringBuilder writer, String name, String value)
            throws IOException {
        if (value != null && value.length() > 0) {
            writer.append(name);
            writer.append(": ");
    ...
    public static void writeAddressHeaderWithQP(StringBuilder writer, String name, String value)
            throws IOException {
        if (value != null && value.length() > 0) {
            writer.append(name);
            writer.append(": =?UTF-8?Q?");
            writer.append(encodeQForm(Address.packedToHeader(value)));
            writer.append("?=<");
            writer.append(Address.packedToHeader(value));
            writer.append(">\r\n");
        }
    ...
    /// M: for QP encode
    /**
     * Encodes English Latters on From, To, CC, ......
     */
    public static String encodeQForm(String text) throws IOException{
        ...
        for (int idx = 0; idx < bytes.length; idx++) {
            ...
            if(bytes[idx] >= 33 && bytes[idx] <= 64 && bytes[idx] != 61) {
                sb.append((char)bytes[idx]);
            }
            else if(bytes[idx] >= 91 && bytes[idx] <= 96) {
                sb.append((char)bytes[idx]);
            }
            else if(bytes[idx] >= 123 && bytes[idx] <= 126) {
                sb.append((char)bytes[idx]);
            }
            else {
                sb.append('=');
     ...
     */
    public static String encodeQ(String text) throws IOException{
        if(text == null)
        {
            return null;
        }
        ...
        for (int idx = 0; idx < bytes.length; idx++) {
            if(bytes[idx] >= 33 && bytes[idx] <= 126 && bytes[idx] != 61)
            {
                sb.append((char)bytes[idx]);
            }
            else if(bytes[idx] == 32)
            {
                sb.append((char)bytes[idx]);
            }
            else
