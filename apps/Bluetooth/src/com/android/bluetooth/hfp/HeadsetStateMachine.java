...
final class HeadsetStateMachine extends StateMachine {
    ...
    /// M: The headset HBH-DS205 can't establish SCO successfully when A2DP exists.
            ...
            switch(message.what) {
                ...
                case QUERY_PHONE_STATE_TIMEOUT:
                {
                   if (mPhoneProxy != null) {
                       try {
                           ...
                           Log.d(TAG, "set QUERY_PHONE_STATE_TIMEOUT again!!");
                ...
                case STACK_EVENT:
                    ...
                    switch (event.type) {
                        ...
                        case EVENT_TYPE_HF_BATTERY_LEVEL:
                            processHFBatteryLevel(event.valueInt, event.device);
                            break;
                        case EVENT_TYPE_HF_ENHANCED_SAFETY:
                            processHFEnhancedSafety(event.valueInt, event.device);
                            break;
                        /// @}
                        default:
    ...
    private void processDialCall(String number, BluetoothDevice device) {
        ...
        /// M: SIP Call Dial support @{
        String scheme = SCHEME_TEL;
        if (PhoneNumberUtils.isUriNumber(dialNumber)) {
            if (DBG) {
                log("The number is SIP call");
            }
            scheme = SCHEME_SIP;
        }

        Intent intent = new Intent(Intent.ACTION_CALL_PRIVILEGED,
                                   Uri.fromParts(scheme, dialNumber, null));
        //Intent intent = new Intent(Intent.ACTION_CALL_PRIVILEGED,
    ...
    private void processCallState(HeadsetCallState callState,
        ...
        if (isVirtualCall) {
            ...
            String oldHeadsetName = "HBH-DS205";

            if (mCurrentDevice != null) {
                deviceName = mCurrentDevice.getName();

                if (deviceName != null && deviceName.equals(oldHeadsetName)) {
                    // HBH-DS205
                    if (callState.mNumActive == 0 && callState.mNumHeld == 0
            ...
            if (callState.mCallState == HeadsetHalConstants.CALL_STATE_IDLE
    ...
    /// M: HF indicator support @{
    private void processHFBatteryLevel(int batteryLevel, BluetoothDevice device) {
        // broadcast the value of battery level
        Intent intent = new Intent(BluetoothHeadset.ACTION_HF_BATTERY_LEVEL_EVENT);
        intent.putExtra(BluetoothHeadset.EXTRA_HF_BATTERY_LEVEL_EVENT_VALUE, batteryLevel);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, device);
        mService.sendBroadcastAsUser(intent, UserHandle.ALL,
                    HeadsetService.BLUETOOTH_PERM);
        log("Broadcast HF BatteryLevel " + device + ": " + batteryLevel);
    }

    private void processHFEnhancedSafety(int enhancedSafety, BluetoothDevice device) {
        // enhancedSafety, 0: no; 1:yes
        // broadcast the value of enhanced safety
        Intent intent = new Intent(BluetoothHeadset.ACTION_HF_ENHANCED_SAFETY_EVENT);
        intent.putExtra(BluetoothHeadset.EXTRA_HF_ENHANCED_SAFETY_EVENT_VALUE, enhancedSafety);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, device);
        mService.sendBroadcastAsUser(intent, UserHandle.ALL,
                    HeadsetService.BLUETOOTH_PERM);
        log("Broadcast HF enhanceSafety " + device + ": " + enhancedSafety);
    }
    /// @}
    ...
    /// M: HF indicator support @{
    private void onHFBatteryLevel(int batteryLevel, byte[] address) {
        StackEvent event = new StackEvent(EVENT_TYPE_HF_BATTERY_LEVEL);
        event.valueInt = batteryLevel;
        event.device = getDevice(address);
        sendMessage(STACK_EVENT, event);
    }

    private void onHFEnhancedSafety(int enhancedSafety, byte[] address) {
        StackEvent event = new StackEvent(EVENT_TYPE_HF_ENHANCED_SAFETY);
        event.valueInt = enhancedSafety;
        event.device = getDevice(address);
        sendMessage(STACK_EVENT, event);
    }
    /// @}
    ...
    /// M: HF indicator support @{
    final private static int EVENT_TYPE_HF_BATTERY_LEVEL = 30;
    final private static int EVENT_TYPE_HF_ENHANCED_SAFETY = 31;
    /// @}
