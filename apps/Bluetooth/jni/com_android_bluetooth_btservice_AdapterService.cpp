...
static jboolean configSspDebugModeNative(JNIEnv* env, jobject obj, jboolean enable) {
    ALOGV("%s:",__FUNCTION__);

    jboolean result = JNI_FALSE;
    if (!sBluetoothInterface){
        ALOGV("%s: result=%d", __FUNCTION__, result);
        return result;
    }

#if defined(MTK_BT_COMMON)
    int ret = sBluetoothInterface->config_ssp_debug_mode(enable);

    result = (ret == BT_STATUS_SUCCESS) ? JNI_TRUE : JNI_FALSE;
#else
    ALOGV("%s: MTK_BT_COMMON is off", __FUNCTION__);
#endif/* MTK_BT_COMMON */
...
static JNINativeMethod sMethods[] = {
    ...
    {"configSspDebugModeNative", "(Z)Z", (void*) configSspDebugModeNative},
