...
#ifdef MTK_BT_HFP_AG_1_7
static jmethodID method_onEnhancedSafety;
...
static void connection_state_callback(bthf_connection_state_t state, bt_bdaddr_t* bd_addr) {
    ...
    sCallbackEnv->SetByteArrayRegion(addr, 0, sizeof(bt_bdaddr_t), (jbyte*) bd_addr);
    /// M: @{ALPS02441303 check mCallbacjObj nullness
...
#ifdef MTK_BT_HFP_AG_1_7
static void hf_enhanced_safety_callback(bthf_biev_safety_type_t safety, bt_bdaddr_t* bd_addr) {
    jbyteArray addr;
    ...
    sCallbackEnv->SetByteArrayRegion(addr, 0, sizeof(bt_bdaddr_t), (jbyte *) bd_addr);
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onEnhancedSafety, (jint) safety, addr);
    checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
    sCallbackEnv->DeleteLocalRef(addr);
}

static void hf_battery_level_callback(int battery_level, bt_bdaddr_t* bd_addr) {
    jbyteArray addr;
    ...
    sCallbackEnv->SetByteArrayRegion(addr, 0, sizeof(bt_bdaddr_t), (jbyte *) bd_addr);
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onBatteryLevel, (jint) battery_level, addr);
    checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
    sCallbackEnv->DeleteLocalRef(addr);
}
#endif
...
static bthf_callbacks_t sBluetoothHfpCallbacks = {
    ...
    hf_enhanced_safety_callback,
    hf_battery_level_callback
#else
    key_pressed_callback
#endif
};
...
static void classInitNative(JNIEnv* env, jclass clazz) {
 ...
 #ifdef MTK_BT_HFP_AG_1_7
    method_onEnhancedSafety = env->GetMethodID(clazz, "onHFEnhancedSafety", "(I[B)V");
    method_onBatteryLevel = env->GetMethodID(clazz, "onHFBatteryLevel", "(I[B)V");
 #endif
    /*
