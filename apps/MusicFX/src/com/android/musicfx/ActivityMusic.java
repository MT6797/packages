    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            if (action.equals(Intent.ACTION_HEADSET_PLUG)) {
                Log.v(TAG, "audioManager.isBluetoothA2dpOn() " + audioManager.isBluetoothA2dpOn()
                + " intent.getIntExtra(state, 0) " + intent.getIntExtra("state", 0) +
                ...
                if ((deviceClass == BluetoothClass.Device.AUDIO_VIDEO_HEADPHONES)
                        || (deviceClass == BluetoothClass.Device.AUDIO_VIDEO_WEARABLE_HEADSET)) {
                    Log.v(TAG, "audioManager.isWiredHeadsetOn()" + audioManager.isWiredHeadsetOn());
                    mIsHeadsetOn = audioManager.isWiredHeadsetOn();
                }
            }
...
//        }
    ...
    Log.v(TAG, "updateUIHeadset");
        if (mToggleSwitch.isChecked()) {
            ...
            Log.v(TAG, "mIsHeadsetOn = " + mIsHeadsetOn +
            ", mVirtualizerIsHeadphoneOnly = " + mVirtualizerIsHeadphoneOnly);
        }
    }
    ...
    private void equalizerBandsInit(View eqcontainer) {
        ...
        for (int band = 0; band < mNumberEqualizerBands; band++) {
            ...
            mEqualizerSeekBar[band] = (SeekBar) eqcontainer
                    ...
                    if (mEqualizerSeekBar[band] != null)
            {
        ...
        for (int band = mNumberEqualizerBands; band < EQUALIZER_MAX_BANDS; band++) {
            ...
            // SeekBar
            if (eqcontainer.findViewById(EQViewElementIds[band][1]) != null)
    ...
    private void presetReverbSetPreset(final int preset) {
        ...
        if (preset == PRESET_REVERB_NONE) {
            /// If preset reverb is none,send intend to music to
            ///detach reverb effect from media player
            /// and set reverb effect disabled
            Intent intent = new Intent(DETACH_AUX_AUDIO_EFFECT);
            intent.putExtra(AUX_AUDIO_EFFECT_ID,
                            ControlPanelEffect.getAuxiliaryEffectId(mAudioSession));
            ...
            /// If preset reverb is not none,send intend to music
        ...
        /// @}
        Log.d(TAG, "presetReverb = " + preset + " ,enabled = "
        + ControlPanelEffect.getReverbEnabled(mAudioSession));
    }
