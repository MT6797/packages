...
public class TestDialerActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /// M: for video call @{
        findViewById(R.id.place_video_call_button).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                placeCall(true);
            }
    ...
    /**
     * M: add param to support video call.
     * @param isVideoCall
     */
    private void placeCall(boolean isVideoCall) {
        final TelecomManager telecomManager =
                (TelecomManager) getSystemService(Context.TELECOM_SERVICE);
        telecomManager.placeCall(Uri.fromParts(PhoneAccount.SCHEME_TEL,
                mNumberView.getText().toString(), null), createCallIntentExtras(isVideoCall));
    }
    ...
    /// M: add param to support video call
    private Bundle createCallIntentExtras(boolean isVideoCall) {
        ...
        /// M: add param to support video call @{
        if (isVideoCall) {
            intentExtras.putInt(TelecomManager.EXTRA_START_CALL_WITH_VIDEO_STATE,
